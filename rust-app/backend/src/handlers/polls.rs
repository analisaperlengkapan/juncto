...
pub fn create_poll(user_id: &str, mut poll: Poll, state: &Arc<AppState>) -> Result<ServerMessage, String> {
    let is_host = {
...
pub fn vote(user_id: &str, poll_id: String, option_id: u32, state: &Arc<AppState>) -> Result<ServerMessage, String> {
    let mut polls = state.polls.lock().unwrap();
...
mod tests {
    use super::*;
    use shared::{RoomConfig, PollOption};
    ...
    fn test_vote() {
...

        {
            ...
            polls.insert(poll_id.clone(), Poll {
                ...
                options: vec![
                    PollOption { id: 1, text: "A".to_string(), votes: 0 },
                    PollOption { id: 2, text: "B".to_string(), votes: 0 },
                ],
                voters: HashSet::new(),
            });
        }
