/* global APP, JunctoMeetJS, config, interfaceConfig */

import { junctoLocalStorage } from '@juncto/js-utils';
import Logger from '@juncto/logger';
...
import {
    JunctoConferenceErrors,
    JunctoConferenceEvents,
    JunctoE2ePingEvents,
    JunctoMediaDevicesEvents,
    JunctoTrackEvents,
    browser
} from './react/features/base/lib-juncto';
import {
    gumPending,
    setAudioAvailable,
    setAudioMuted,
...
import {
    createLocalTracksF,
    getLocalJunctoAudioTrack,
    getLocalJunctoVideoTrack,
    getLocalVideoTrack,
    isLocalTrackMuted,
    isUserInteractionRequiredForUnmute
} from './react/features/base/tracks/functions';
import { getLocalJunctoAudioTrackSettings } from './react/features/base/tracks/functions.web';
import { downloadJSON } from './react/features/base/util/downloadJSON';
import { getJunctoMeetGlobalNSConnectionTimes } from './react/features/base/util/helpers';
...
class ConferenceConnector {
    ...
    constructor(resolve, reject, conference) {
        ...
        room.on(JunctoConferenceEvents.CONFERENCE_JOINED,
            this._handleConferenceJoined.bind(this));
        room.on(JunctoConferenceEvents.CONFERENCE_FAILED,
    ...
    _onConferenceFailed(err, ...params) {
        ...
        case JunctoConferenceErrors.RESERVATION_ERROR: {
        ...
        case JunctoConferenceErrors.GRACEFUL_SHUTDOWN:
            APP.store.dispatch(showErrorNotification({
        ...
        // because Juncofo is not available, but it is going to give it another
        // try.
        case JunctoConferenceErrors.FOCUS_DISCONNECTED: {
        ...
        case JunctoConferenceErrors.FOCUS_LEFT:
        case JunctoConferenceErrors.ICE_FAILED:
        case JunctoConferenceErrors.VIDEOBRIDGE_NOT_AVAILABLE:
        case JunctoConferenceErrors.OFFER_ANSWER_FAILED:
        ...
        case JunctoConferenceErrors.INCOMPATIBLE_SERVER_VERSIONS:
    ...
    _unsubscribe() {
        room.off(
            JunctoConferenceEvents.CONFERENCE_JOINED,
            this._handleConferenceJoined);
        room.off(
            JunctoConferenceEvents.CONFERENCE_FAILED,
    ...
    connect() {
        ...
        // the local storage overrides here and in connection.js can be used by junbri
        room.join(junctoLocalStorage.getItem('xmpp_conference_password_override'), replaceParticipant);
    }
}
...
export default {
     ...
     * @returns {Promise<JunctoLocalTrack[]>, Object}
     */
    ...
    async init({ roomName, shouldDispatchConnect }) {
        ...
        const connectionTimes = getJunctoMeetGlobalNSConnectionTimes();
     ...
     * @returns {JunctoParticipant[]}
     *
     * NOTE: Used by juncto-torture!
     */
    listMembers() {
    ...
    /**
     * Used by Junbri to detect when it's alone and the meeting should be terminated.
     */
    get membersCount() {
        return room.getParticipants()
    ...
    /**
     * Finds JunctoParticipant for given id.
     *
     * @param {string} id participant's identifier(MUC nickname).
     *
     * @returns {JunctoParticipant|null} participant instance for given id or
     * null if not found.
     */
     ...
     * decision whether the Command(s) API of room (i.e. lib-juncto's
     * JunctoConference) is to be used in the implementation of the Command(s)
    ...
    _createRoom(localTracks) {
        room = APP.connection.initJunctoConference(APP.conference.roomName, this._getConferenceOptions());
     ...
     * @param {JunctoLocalTrack} newTrack - new track to use or null
     * @returns {Promise}
     */
    useVideoStream(newTrack) {
        ...
        return new Promise((resolve, reject) => {
            _replaceLocalVideoTrackQueue.enqueue(onFinish => {
                const state = APP.store.getState();
                const oldTrack = getLocalJunctoVideoTrack(state);
    ...
    useAudioStream(newTrack) {
        return new Promise((resolve, reject) => {
            _replaceLocalAudioTrackQueue.enqueue(onFinish => {
                const oldTrack = getLocalJunctoAudioTrack(APP.store.getState());
    ...
    async _turnScreenSharingOff(didHaveVideo, ignoreDidHaveVideo) {
        ...
        const duration = getLocalVideoTrack(tracks)?.junctoTrack.getDuration() ?? 0;

        // If system audio was also shared stop the AudioMixerEffect and dispose of the desktop audio track.
        if (this._mixerEffect) {
            const localAudio = getLocalJunctoAudioTrack(APP.store.getState());
    ...
    /**
     * Creates desktop (screensharing) {@link JunctoLocalTrack}
     *
     * @return {Promise.<JunctoLocalTrack>} - A Promise resolved with
     * {@link JunctoLocalTrack} for the screensharing or rejected with
     * {@link JunctoTrackError}.
     *
     * @private
     */
    _createDesktopTrack() {
        ...
        return getDesktopStreamPromise.then(desktopStreams => {
            ...
            if (desktopAudioStream) {
                desktopAudioStream.on(
                    JunctoTrackEvents.LOCAL_TRACK_STOPPED,
                    () => {
    ...
    _setupListeners() {
        ...
        room.on(
            JunctoConferenceEvents.CONFERENCE_JOIN_IN_PROGRESS,
            () => APP.store.dispatch(conferenceJoinInProgress(room)));

        room.on(
            JunctoConferenceEvents.CONFERENCE_LEFT,
            (...args) => {
        ...
        room.on(
            JunctoConferenceEvents.CONFERENCE_UNIQUE_ID_SET,
            (...args) => {
        ...
        if (!config.tokenAuthUrl) {
            room.on(
                JunctoConferenceEvents.AUTH_STATUS_CHANGED,
                (authEnabled, authLogin) =>
                    APP.store.dispatch(authStatusChanged(authEnabled, authLogin)));
        }

        room.on(JunctoConferenceEvents.PARTCIPANT_FEATURES_CHANGED, user => {
            APP.store.dispatch(updateRemoteParticipantFeatures(user));
        });
        room.on(JunctoConferenceEvents.USER_JOINED, (id, user) => {
        ...
        room.on(JunctoConferenceEvents.USER_LEFT, (id, user) => {
            // The logic shared between RN and web.
        ...
        room.on(JunctoConferenceEvents.USER_STATUS_CHANGED, (id, status) => {
        ...
        room.on(JunctoConferenceEvents.USER_ROLE_CHANGED, (id, role) => {
        ...
        room.on(JunctoConferenceEvents.TRACK_ADDED, track => {
        ...
        room.on(JunctoConferenceEvents.TRACK_REMOVED, track => {
            if (!track || track.isLocal()) {
                return;
            }
        ...
        room.on(JunctoConferenceEvents.TRACK_AUDIO_LEVEL_CHANGED, (id, lvl) => {
            const localAudio = getLocalJunctoAudioTrack(APP.store.getState());
            let newLvl = lvl;
        ...
        room.on(JunctoConferenceEvents.TRACK_MUTE_CHANGED, (track, participantThatMutedUs) => {
            if (participantThatMutedUs) {
                APP.store.dispatch(participantMutedUs(participantThatMutedUs, track));
            }
        });

        room.on(JunctoConferenceEvents.TRACK_UNMUTE_REJECTED, track => APP.store.dispatch(destroyLocalTracks(track)));

        room.on(JunctoConferenceEvents.SUBJECT_CHANGED,
            subject => APP.store.dispatch(conferenceSubjectChanged(subject)));

        room.on(
            JunctoConferenceEvents.LAST_N_ENDPOINTS_CHANGED,
            (leavingIds, enteringIds) =>
                APP.UI.handleLastNEndpoints(leavingIds, enteringIds));

        room.on(
            JunctoConferenceEvents.P2P_STATUS,
            (junctoConference, p2p) =>
                APP.store.dispatch(p2pStatusChanged(p2p)));

        room.on(
            JunctoConferenceEvents.DOMINANT_SPEAKER_CHANGED,
            (dominant, previous, silence) => {
                APP.store.dispatch(dominantSpeakerChanged(dominant, previous, Boolean(silence), room));
            });

        room.on(
            JunctoConferenceEvents.CONFERENCE_CREATED_TIMESTAMP,
            conferenceTimestamp => {
        ...
        room.on(
            JunctoConferenceEvents.DISPLAY_NAME_CHANGED,
            (id, displayName) => {
        ...
        room.on(
            JunctoConferenceEvents.SILENT_STATUS_CHANGED,
            (id, isSilent) => {
                APP.store.dispatch(participantUpdated({
                    conference: room,
                    id,
        ...
        room.on(
            JunctoConferenceEvents.BOT_TYPE_CHANGED,
            (id, botType) => {
        ...
        room.on(
            JunctoConferenceEvents.TRANSCRIPTION_STATUS_CHANGED,
            (status, id, abruptly) => {
                if (status === JunctoMeetJS.constants.transcriptionStatus.ON) {
                    APP.store.dispatch(transcriberJoined(id));
                } else if (status === JunctoMeetJS.constants.transcriptionStatus.OFF) {
                    APP.store.dispatch(transcriberLeft(id, abruptly));
                }
            });

        room.on(
            JunctoConferenceEvents.ENDPOINT_MESSAGE_RECEIVED,
            (participant, data) => {
        ...
        room.on(
            JunctoConferenceEvents.NON_PARTICIPANT_MESSAGE_RECEIVED,
            (id, data) => {
        ...
        room.on(
            JunctoConferenceEvents.LOCK_STATE_CHANGED,
            (...args) => APP.store.dispatch(lockStateChanged(room, ...args)));

        room.on(
            JunctoConferenceEvents.PROPERTIES_CHANGED,
            properties => APP.store.dispatch(conferencePropertiesChanged(properties)));

        room.on(JunctoConferenceEvents.KICKED, (participant, reason, isReplaced) => {
            if (isReplaced) {
        ...
        room.on(JunctoConferenceEvents.PARTICIPANT_KICKED, (kicker, kicked) => {
            APP.store.dispatch(participantKicked(kicker, kicked));
        });

        room.on(JunctoConferenceEvents.PARTICIPANT_SOURCE_UPDATED,
            junctoParticipant => {
                APP.store.dispatch(participantSourcesUpdated(junctoParticipant));
            });

        room.on(JunctoConferenceEvents.SUSPEND_DETECTED, () => {
            APP.store.dispatch(suspendDetected());
        });

        room.on(
            JunctoConferenceEvents.AUDIO_UNMUTE_PERMISSIONS_CHANGED,
            disableAudioMuteChange => {
                APP.store.dispatch(setAudioUnmutePermissions(disableAudioMuteChange));
            });
        room.on(
            JunctoConferenceEvents.VIDEO_UNMUTE_PERMISSIONS_CHANGED,
            disableVideoMuteChange => {
                APP.store.dispatch(setVideoUnmutePermissions(disableVideoMuteChange));
            });

        room.on(
            JunctoE2ePingEvents.E2E_RTT_CHANGED,
        ...
        room.on(
            JunctoConferenceEvents.START_MUTED_POLICY_CHANGED,
            ({ audio, video }) => {
        ...
        room.on(
            JunctoConferenceEvents.DATA_CHANNEL_OPENED, () => {
        ...
        room.on(
            JunctoConferenceEvents.DATA_CHANNEL_CLOSED, ev => {
                const state = APP.store.getState();
        ...
        room.on(JunctoConferenceEvents.PERMISSIONS_RECEIVED, p => {
            const localParticipant = getLocalParticipant(APP.store.getState());
    ...
    async onAudioDeviceChanged(micDeviceId) {
        ...
        .then(() => {
            const state = APP.store.getState();
            const localAudio = getLocalJunctoAudioTrack(state);
            const settings = getLocalJunctoAudioTrackSettings(state);
    ...
    onVideoDeviceChanged(cameraDeviceId) {
        ...
        const localVideoTrack = getLocalJunctoVideoTrack(APP.store.getState());
    ...
    onSuspendDetected() {
        ...
        if (this.deviceChangeListener) {
            JunctoMeetJS.mediaDevices.removeEventListener(
                JunctoMediaDevicesEvents.DEVICE_LIST_CHANGED,
    ...
    _initDeviceList(setDeviceListChangeHandler = false) {
        const { mediaDevices } = JunctoMeetJS;
    ...
    /**
     * Event listener for JunctoMediaDevicesEvents.DEVICE_LIST_CHANGED to
    ...
    async _onDeviceListChanged(devices) {
        ...
        const localAudio = getLocalJunctoAudioTrack(state);
        const localVideo = getLocalJunctoVideoTrack(state);
    ...
    updateVideoIconEnabled() {
        ...
        const localVideo = getLocalJunctoVideoTrack(APP.store.getState());
    ...
    onProxyConnectionEvent(event) {
        if (!this._proxyConnection) {
            this._proxyConnection = new JunctoMeetJS.ProxyConnectionService({

                /**
                 * Pass the {@code JunctoConnection} instance which will be used
                 * to fetch TURN credentials.
                 */
                junctoConnection: APP.connection,
                 ...
                 * desktop stream.
                 *
                 * @param {JunctoLocalTrack} remoteProxyStream - The media
