...
JUNCTO_INSTALLATION="DEBIAN"
JAAS_ENDPOINT="https://account-provisioning.cloudflare.juncto.net/operations"
CHALLENGE_FILE="/usr/share/juncto/.well-known/juncto-challenge.txt"
SUPPORT_MSG="Reach out to JunctoService support or retry with /usr/share/juncto/scripts/register-jaas-account.sh"

create_error=0
create_data=$(curl -s -f -X 'POST' "${JAAS_ENDPOINT}" -H 'Content-Type: application/json' -H 'accept: */*' \
  -d "{ \"domain\": \"${DOMAIN}\", \"email\": \"${EMAIL}\", \"junctoInstallation\": \"${JUNCTO_INSTALLATION}\" }") || create_error=$?
...
(while true; do
    ...
    if [ "${status}" == "PROVISIONED" ]; then
        ...
        echo "=================="
        echo ""
        echo "JunctoService account was created. To finish setup follow the email that was sent."
        echo ""
        echo "=================="
        exit 0;
        ...
        echo "=================="
        echo ""
        echo "JunctoService account creation failed:${provisioned_data}"
        echo ""
        echo "=================="
        exit 4
