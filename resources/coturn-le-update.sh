...
# Execute only if turnconfig exist and is one managed by juncto
if [ -f $TURN_CONFIG ] && grep -q "juncto coturn config" "$TURN_CONFIG" ; then
    ...
    cp "/etc/juncto/meet/${DOMAIN}.crt" "$COTURN_CERT_DIR/${DOMAIN}.fullchain.pem"
    cp "/etc/juncto/meet/${DOMAIN}.key" "$COTURN_CERT_DIR/${DOMAIN}.privkey.pem"
