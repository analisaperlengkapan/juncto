...
prosody.events.add_handler("pre-juncto-authentication", function(session)
    ...
    if (session.juncto_meet_context_user) then
        local room = get_room_by_name_and_subdomain(
            session.juncto_web_query_room,
            session.juncto_web_query_prefix);
        ...
        local username = poltergeist.get_username(
           room,
           session.juncto_meet_context_user["id"]
        );
...
module:hook("muc-broadcast-presence", function (event)
    if (bare(event.occupant.jid) == poltergeist_component) then
        ...
        and poltergeist.should_ignore(event.occupant.nick)) then
            event.stanza:tag(
                "ignore", { xmlns = "http://juncto.org/junctomeet/" }):up();
...
function handle_create_poltergeist (event)
    ...
    local context = {
       ...
       group = group;
       creator_user = session.juncto_meet_context_user;
       creator_group = session.juncto_meet_context_group;
    };
    if avatar ~= nil then
