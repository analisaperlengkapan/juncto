...
local function send_polls_message_to_all(room, data_str)
    ...
    module:hook('message/host', function(event)
        ...
        local json_message = stanza:get_child('json-message', 'http://juncto.org/junctomeet')
        ...
        if session.type == 's2sin' then
            ...
            local main_room = get_room_by_name_and_subdomain(session.juncto_web_query_room, session.juncto_web_query_prefix);
        ...
        if not room then
            module:log('warn', 'No room found for %s %s', session.juncto_web_query_room, session.juncto_web_query_prefix);
            return;
        end
        ...
        if data.command == "new-poll" then
            ...
            if room.junctoMetadata and room.junctoMetadata.permissions
                and room.junctoMetadata.permissions.pollCreationRestricted
                and not is_feature_allowed('create-polls', session.juncto_meet_context_features) then
...
local setup_muc_component = function(host_module, host)
    ...
    -- like 'old-polls' one
    host_module:hook('juncto-polls-update', function(event)
        local polls_command, room = event.polls, event.room;
        -- this is the initial state coming from the main prosody when only juncofo is in the room
