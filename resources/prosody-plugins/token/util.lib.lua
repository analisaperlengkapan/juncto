...
-- Copyright (C) 2021-present Juncto, Inc.

local basexx = require "basexx";
local have_async, async = pcall(require, "util.async");
local hex = require "util.hex";
local jwt = module:require "luajwtjuncto";
...
local ASAPIssuer = module:get_option_string('asap_issuer', 'juncto');
local ASAPAudience = module:get_option_string('asap_audience', 'juncto');
local ASAPKeyId = module:get_option_string('asap_key_id', 'juncto');
...
-- session.juncto_meet_room - the room name value from the token
-- session.juncto_meet_domain - the domain name value from the token
-- session.juncto_meet_context_user - the user details from the token
-- session.juncto_meet_context_room - the room details from the token
-- session.juncto_meet_context_group - the group value from the token
-- session.juncto_meet_context_features - the features value from the token
...
function Util:process_and_verify_token(session)
    ...
    if claims ~= nil then
        ...
        session.juncto_meet_room = claims["room"];
        -- Binds domain name to the session
        session.juncto_meet_domain = claims["sub"];
        session.juncto_meet_auth_issuer = claims["iss"];

        -- Binds the user details to the session if available
        if claims["context"] ~= nil then
          session.juncto_meet_str_tenant = claims["context"]["tenant"];

          if claims["context"]["user"] ~= nil then
            session.juncto_meet_context_user = claims["context"]["user"];
          end

          if claims["context"]["group"] ~= nil then
            -- Binds any group details to the session
            session.juncto_meet_context_group = claims["context"]["group"];
          end

          if claims["context"]["features"] ~= nil then
            -- Binds any features details to the session
            session.juncto_meet_context_features = claims["context"]["features"];
          end
          if claims["context"]["room"] ~= nil then
            session.juncto_meet_context_room = claims["context"]["room"]
          end
        elseif claims["user_id"] then
          session.juncto_meet_context_user = {};
          session.juncto_meet_context_user.id = claims["user_id"];
        end

        -- fire event that token has been verified and pass the session and the decoded token
        prosody.events.fire_event('juncto-authentication-token-verified', {
            session = session;
            claims = claims;
        });
...
function Util:verify_room(session, room_address)
    ...
    local auth_room = session.juncto_meet_room;
    if auth_room then
        if type(auth_room) == 'string' then
            auth_room = string.lower(auth_room);
        else
            module:log('warn', 'session.juncto_meet_room not string: %s', inspect(auth_room));
        end
    end
    ...
    if auth_room == '*' then
        ...
        if session.juncto_meet_context_room and (session.juncto_meet_context_room["regex"] == true or session.juncto_meet_context_room["regex"] == "true") then
            if target_room ~= nil then
                -- room with subdomain
    ...
    if session.juncto_meet_str_tenant
        and string.lower(session.juncto_meet_str_tenant) ~= session.juncto_web_query_prefix then
        session.juncto_meet_tenant_mismatch = true;
        ...
        module:log('warn', 'Tenant differs for user:%s group:%s url_tenant:%s token_tenant:%s',
            session.juncto_meet_context_user and session.juncto_meet_context_user.id or '',
            session.juncto_meet_context_group,
            session.juncto_web_query_prefix, session.juncto_meet_str_tenant);
    end

    local auth_domain = string.lower(session.juncto_meet_domain);
