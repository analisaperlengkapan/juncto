...
local PARTICIPANT_PROP_RAISE_HAND = 'juncto_participant_raisedHand';
local PARTICIPANT_PROP_REQUEST_TRANSCRIPTION = 'juncto_participant_requestingTranscription';
local PARTICIPANT_PROP_TRANSLATION_LANG = 'juncto_participant_translation_language';
...
local DISPLAY_NAME_NS = 'http://juncto.org/protocol/display-name';
...
local function send_transcriptions_update(room)
    ...
    module:send(st.iq({
        ...
        from = local_domain,
        id = iq_id })
      :tag('visitors', { xmlns = 'juncto:visitors',
    ...
    if prosody.hosts[host] then
        ...
        if room._main_room_lobby_enabled then
            origin.send(st.error_reply(stanza, 'cancel', 'not-allowed', 'Visitors not allowed while lobby is on!')
                :tag('no-visitors-lobby', { xmlns = 'juncto:visitors' }));
            return true;
        else
...
-- when occupant is leaving forward presences to juncofo for visitors
...
module:hook('muc-occupant-left', function (event)
    ...
    if prosody.hosts[occupant_domain] and not is_admin(occupant.bare_jid) then
        ...
        -- Let's forward unavailable presence to the special juncofo
    ...
    -- we can destroy and the visitor one as when juncofo leaves all visitors will reload
...
-- forward visitor presences to juncofo
...
module:hook('muc-broadcast-presence', function (event)
    ...
    ---- we are interested only of visitors presence to send it to juncofo
    if occupant.role ~= 'visitor' then
        return;
    end
    ...
    -- create a presence to send it to juncofo, as juncofo is special :)
    ...
    if raiseHand then
        ...
        local identity = session and session.juncto_meet_context_user;
        ...
        if is_vpaas(room) and identity then
            ...
            if identity and identity.id then
                user_id = session.juncto_meet_context_user.id;
                group_id = session.juncto_meet_context_group;

                if session.auth_token and auto_promoted_with_token then
                    if not session.juncto_meet_tenant_mismatch or session.juncto_web_query_prefix == '' then
    ...
    for room in prosody.hosts[module.host].modules.muc.each_room() do
        ...
        for _, o in room:each_occupant() do
            if not is_admin(o.bare_jid) then
                ...
                if prosody.hosts[host] then -- local hosts are visitors (including jungasi)
                    visitors_count = visitors_count + 1;
                else
...
module:hook('juncofo-unlock-room', function(e)
...
-- disconnect - sent when main room is destroyed or when we receive a 'disconnect-vnode' iq from juncofo
-- update - sent on:
...
local function iq_from_main_handler(event)
    ...
    if files then
        ...
        local removed, added = table_compare(room.juncto_shared_files or {}, received_files)
        for _, id in ipairs(removed) do
            module:context(local_domain):fire_event('juncto-filesharing-remove', {
                room = room; id = id;
            });
        end
        for _, id in ipairs(added) do
            module:context(local_domain):fire_event('juncto-filesharing-add', {
    ...
    if pollsEl then
        ...
        module:fire_event('juncto-polls-update', { room = room; polls = polls; });
    end

    if fire_juncofo_unlock then
        -- everything is connected allow participants to join
        module:fire_event('juncofo-unlock-room', { room = room; fmuc_fired = true; });
    end
...
function filter_stanza(stanza, session)
    ...
    elseif stanza.name == 'presence' and session.type == 'c2s' and jid.node(stanza.attr.to) == 'focus' then
        ...
        -- we want to filter presences to juncofo for the main participants, skipping visitors
...
function route_s2s_stanza(event)
    ...
    if to_host ~= main_domain then
     ...
     if stanza.name == 'presence' then
        ...
        -- all other presences from juncofo or the main participants there is no point to go to the main node
...
module:hook('answer-poll', function(answerData)
    ...
    :tag("json-message", { xmlns = "http://juncto.org/junctomeet"; roomJid = room_jid; })
    :text(data_str)
    :up();
