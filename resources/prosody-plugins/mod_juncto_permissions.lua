...
local function load_config()
    default_permissions = module:get_option('juncto_default_permissions', {
        livestreaming = true;
        recording = true;
        transcription = true;
        ['outbound-call'] = true;
...
function process_set_affiliation(event)
    ...
    if is_admin(jid) or is_healthcheck_room(room.jid) or not actor or not previous_affiliation
        or not actor_session or not actor_session.juncto_meet_context_features then
        return;
    end
    ...
    if (previous_affiliation == 'none' or previous_affiliation == 'member') and affiliation == 'owner' then
        occupant_session.juncto_meet_context_features = actor_session.juncto_meet_context_features;
        if actor_session.juncto_meet_context_user then
            occupant_session.granted_juncto_meet_context_user_id = actor_session.juncto_meet_context_user['id']
                or actor_session.granted_juncto_meet_context_user_id;
        end
        occupant_session.granted_juncto_meet_context_group_id = actor_session.juncto_meet_context_group
            or actor_session.granted_juncto_meet_context_group_id;
    ...
    elseif previous_affiliation == 'owner' and ( affiliation == 'member' or affiliation == 'none' ) then
        occupant_session.granted_juncto_meet_context_user_id = nil;
        occupant_session.granted_juncto_meet_context_group_id = nil;

        -- on revoke
        if not occupant_session.auth_token then
            occupant_session.juncto_meet_context_features = nil;
        end
    end
end
...
-- 1. user joins the room and is granted moderator by another moderator or juncofo
-- 2. Some module changes the role of the user by using set_affiliation method
-- In cases where authentication is 'anonymous', 'juncto-anonymous', 'internal_hashed', 'internal_plain', 'cyrus' we
...
function filter_stanza(stanza, session)
    ...
    if not session.force_permissions_update then
        if session.auth_token and session.juncto_meet_context_features then -- token and features are set so skip
    ...
    if not session.juncto_meet_context_features then
        session.juncto_meet_context_features = default_permissions;
    end

    room.send_default_permissions_to[bare_to] = nil;

    stanza:tag('permissions', { xmlns='http://juncto.org/junctomeet' });
    for k, v in pairs(session.juncto_meet_context_features) do
