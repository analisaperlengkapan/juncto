...
function generateToken(session, audience, room, occupant)
    ...
    if session.juncto_web_query_prefix and session.juncto_web_query_prefix ~= '' then
        sub = session.juncto_web_query_prefix;
    end
    ...
    local payload = {
        ...
        sub = sub,
        context = {
            group = session.juncto_meet_context_group or session.granted_juncto_meet_context_group_id,
            user = session.juncto_meet_context_user or {
                id = session.full_jid,
            ...
            features = session.juncto_meet_context_features
        },
        room = session.juncto_web_query_room,
        meeting_id = room._data.meetingId,
        granted_from = session.granted_juncto_meet_context_user_id,
        customer_id = id or session.juncto_meet_context_group or session.granted_juncto_meet_context_group_id,
