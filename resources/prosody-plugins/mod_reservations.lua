--- This is a port of Juncofo's Reservation System as a prosody module
--  ref: https://github.com/juncto/juncofo/blob/master/doc/reservation.md
--
--  We try to retain the same behaviour and interfaces where possible, but there
--  is some difference:
--    * In the event that the DELETE call fails, Juncofo's reservation
...
--    * Juncofo's reservation system expect int-based conflict_id. We take any sensible string.
--
...
function RoomReservation:enqueue_or_route_event(event)
    if self.meta.status == STATUS.PENDING then
        ...
        -- and when Juncofo actually creates the room.
    ...
    event.origin.send(
        st.iq({ type="error", to=to, from=from, id=id })
            :tag("error", { type="cancel" })
                ...
                :tag("reservation-error", { xmlns="http://juncto.org/protocol/focus", ["error-code"]=tostring(error_code) })
    );
end
...
function RoomReservation:call_api_create_conference()
    ...
    local http_options = {
        body = http.formencode(request_data);  -- because Juncofo reservation encodes as form data instead JSON
        method = 'POST';
...
--- Intercept conference IQ to Juncofo handle reservation checks before allowing normal event flow
...
module:hook("pre-iq/host", function(event)
    ...
    if stanza.name ~= "iq" or stanza.attr.to ~= focus_component_host or stanza.attr.type ~= 'set' then
        return;  -- not IQ for juncofo. Ignore this event.
    end

    local conference = stanza:get_child('conference', 'http://juncto.org/protocol/focus');
