...
module:depends("juncto_permissions");
...
-- c) Avoids any participant joining the room in the interval between creating the room and juncofo entering the room.
...
-- add the meeting Id in the default config we return to juncofo
...
module:hook("muc-config-submitted/muc#roominfo_meetingId", function(event)
    -- we allow juncofo to overwrite the meetingId
...
local function process_region(session, stanza)
    ...
    local region = stanza:get_child_text('juncto_participant_region');
    if region then
        return;
    end

    stanza:tag('juncto_participant_region'):text(session.user_region):up();
end

--- Avoids any participant joining the room in the interval between creating the room
--- and juncofo entering the room
module:hook('muc-occupant-pre-join', function (event)
    ...
    -- we skip processing only if juncofo_lock is set to false
    if room._data.juncofo_lock == false or is_health_room then
        return;
    end

    if ends_with(occupant.nick, '/focus') then
        module:fire_event('juncofo-unlock-room', { room = room; });
    else
        room._data.juncofo_lock = true;
...
function handle_juncofo_unlock(event)
    local room = event.room;

    room._data.juncofo_lock = false;
    if not room.pre_join_queue then
        return;
    end
...
module:hook('juncofo-unlock-room', handle_juncofo_unlock);
...
local function filterTranscriptionResult(event)
    ...
    local json_message = stanza:get_child_text('json-message', 'http://juncto.org/junctomeet')
    ...
    if msg_obj.type == 'transcription-result' then
        ...
        if msg_obj.transcript ~= nil then
            ...
            return module:fire_event('juncto-transcript-received', {
                room = room, occupant = occupant, transcription = transcription, stanza = stanza });
        end
    end

    return module:fire_event('juncto-endpoint-message-received', {
...
module:hook('juncto-visitor-groupchat-pre-route', filterTranscriptionResult);
