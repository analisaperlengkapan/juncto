...
function filter_stanza_in(stanza, session)
    if not session or not session.juncto_meet_context_features
        or session.juncto_meet_context_features['name-readonly'] ~= true then
    ...
    if not session.juncto_meet_context_user or not session.juncto_meet_context_user.name then
    ...
    if nick_element and nick_element:get_text() ~= session.juncto_meet_context_user.name then
        stanza:remove_children('nick', NICK_NS);
        stanza:tag('nick', { xmlns = NICK_NS }):text(session.juncto_meet_context_user.name):up();
    end
...
module:hook('muc-add-history', function(event)
    ...
    if occupant then
        ...
        if session and session.juncto_meet_context_user then
            if ignore_jwt_name then
                name = occupant:get_presence():get_child_text('nick', NICK_NS);
            else
                name = session.juncto_meet_context_user.name;
            end
