...
function on_message(event)
    ...
    local message = stanza:get_child(FILE_SHARING_IDENTITY_TYPE, 'http://juncto.org/junctomeet');
    ...
    if not room then
        module:log('warn', 'No room found for %s/%s', session.juncto_web_query_prefix, session.juncto_web_query_room);
        return false;
    end
    ...
    if not is_feature_allowed(
        'file-upload',
        session.juncto_meet_context_features,
    ...
    if message.attr.type == JSON_TYPE_ADD_FILE then
        ...
        module:context(muc_domain_base):fire_event('juncto-filesharing-add', {
            room = room; file = msg_obj; actor = occupant.nick;
        });

        module:context(muc_domain_base):fire_event('juncto-filesharing-updated', {
            room = room;
        });
    ...
    elseif message.attr.type == JSON_TYPE_REMOVE_FILE then
        ...
        module:context(muc_domain_base):fire_event('juncto-filesharing-remove', {
...
function occupant_joined(event)
    ...
    if not room.juncto_shared_files
        or is_admin(occupant.bare_jid)
        or not room.juncto_shared_files
        or next(room.juncto_shared_files) == nil then
        return;
    end
    ...
    local json_msg, error = json.encode({
        ...
        files = room.juncto_shared_files
    });

    local stanza = st.message({ from = module.host; to = occupant.jid; })
        :tag('json-message', { xmlns = 'http://juncto.org/junctomeet' })
        :text(json_msg):up();
...
process_host_module(muc_domain_base, function(host_module, host)
    module:context(muc_domain_base):fire_event('juncto-add-identity', {
        name = FILE_SHARING_IDENTITY_TYPE; host = module.host;
    });
    module:context(muc_domain_base):hook('juncto-filesharing-add', function(event)
        local actor, file, room = event.actor, event.file, event.room;

        if not room.juncto_shared_files then
            room.juncto_shared_files = {};
        end

        room.juncto_shared_files[file.fileId] = file;
        ...
        local stanza = st.message({ from = module.host; }):tag('json-message', { xmlns = 'http://juncto.org/junctomeet' })
            :text(json_msg):up();

        -- send add file to all occupants except juncofo and sender
    ...
    module:context(muc_domain_base):hook('juncto-filesharing-remove', function(event)
        ...
        room.juncto_shared_files[id] = nil;
        ...
        -- send remove file to all occupants except juncofo and sender
        -- if this is visitor prosody send it only to visitors
        for _, room_occupant in room:each_occupant() do
            local send_event = not is_admin(room_occupant.bare_jid) and room_occupant.nick ~= actor;
            if is_visitor_prosody then
