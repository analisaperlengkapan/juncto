...
local OUTBOUND_SIP_JIBRI_PREFIXES = { 'outbound-sip-junbri@', 'sipjunbriouta@', 'sipjunbrioutb@' };
local INBOUND_SIP_JIBRI_PREFIXES = { 'inbound-sip-junbri@', 'sipjunbriina@', 'sipjunbriina@' };
local RECORDER_PREFIXES = module:get_option_inherited_set('recorder_prefixes', { 'recorder@recorder.', 'junbria@recorder.', 'junbrib@recorder.' });
...
-- healthcheck rooms in juncofo starts with a string '__juncofo-health-check'
function is_healthcheck_room(room_jid)
    return starts_with(room_jid, "__juncofo-health-check");
end
...
-- Returns the initiator extension if the stanza is coming from a sip jungasi
function is_sip_jungasi(stanza)
    if not stanza then
        return false;
    end

    return stanza:get_child('initiator', 'http://juncto.org/protocol/jungasi');
end

-- This requires presence stanza being passed
function is_transcriber_jungasi(stanza)
    if not stanza then
        return false;
    end
    ...
    for i = 1, #features do
        ...
        if feature.attr and feature.attr.var and feature.attr.var == 'http://juncto.org/protocol/transcriber' then
            return true;
        end
    end
...
function get_sip_junbri_email_prefix(email)
    if not email then
        return nil;
...
function is_sip_junbri_join(stanza)
    if not stanza then
        return false;
    end
    ...
    for i = 1, #features do
        local feature = features[i];
        if feature.attr and feature.attr.var and feature.attr.var == "http://juncto.org/protocol/junbri" then
            if get_sip_junbri_email_prefix(email) then
                module:log("debug", "Occupant with email %s is a sip junbri ", email);
                return true;
            end
        end
    end
...
return {
    ...
    is_sip_junbri_join = is_sip_junbri_join;
    is_sip_jungasi = is_sip_jungasi;
    is_transcriber = is_transcriber;
    is_transcriber_jungasi = is_transcriber_jungasi;
    is_vpaas = is_vpaas;
    get_focus_occupant = get_focus_occupant;
    get_ip = get_ip;
    get_room_from_jid = get_room_from_jid;
    get_room_by_name_and_subdomain = get_room_by_name_and_subdomain;
    get_sip_junbri_email_prefix = get_sip_junbri_email_prefix;
