...
local function verify_user(session, stanza)
    if DEBUG then
        module:log("debug", "Session token: %s, session room: %s",
            tostring(session.auth_token), tostring(session.juncto_meet_room));
    end
    ...
    -- allowlist for participants, jungasi (sip & transcriber), junbri (recorder & sip)
    ...
    if not res then
        ...
        if err then
            response:tag(err, { xmlns = 'http://juncto.org/junctomeet' });
        end
...
for event_name, method in pairs {
    ...
    module:hook(event_name, function (event)
        ...
        -- juncto_meet_room is set after the token had been verified
        if not session.auth_token or not session.juncto_meet_room then
            session.send(
                st.error_reply(
