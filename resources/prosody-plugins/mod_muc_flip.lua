...
-- and the used jwt matches the id(session.juncto_meet_context_user.id) of another user this is indication that the user
...
module:hook("muc-occupant-pre-join", function(event)
    ...
    if session.juncto_meet_context_user and session.juncto_meet_context_user.id then
        local participants = room._data.participants_details or {};
        local id = session.juncto_meet_context_user.id;
        local first_device_occ_nick = participants[id];
        if flip_device_tag then
            if first_device_occ_nick and session.juncto_meet_context_features.flip and (session.juncto_meet_context_features.flip == true or session.juncto_meet_context_features.flip == "true") then
            ...
            elseif not session.juncto_meet_context_features.flip or session.juncto_meet_context_features.flip == false or session.juncto_meet_context_features.flip == "false" then
...
module:hook("muc-occupant-left", function(event)
    ...
    if session and session.juncto_meet_context_user and session.juncto_meet_context_user.id then
        local id = session.juncto_meet_context_user.id
