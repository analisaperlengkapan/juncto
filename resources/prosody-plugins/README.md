...
- juncofo_lock - A boolean value, when set to true the room is locked waiting for Juncofo to join. All attempts to join will be queued until Juncofo joins.
...
- moderators - A list of moderator ids (from the token) that are allowed to join the room. The list is sent to juncofo to take decisions about forwarding to main room or visitor node in case of large meetings. Those participants will be moderators. You need a custom module to set participants as moderators based on the field.
- participants - A list of participants ids (from the token) that are allowed to join the room. The list is sent to juncofo to take decisions about forwarding to main room or visitor node in case of large meetings. Those participants will not be moderators.
...
- _juncto_go_live_sent - A boolean value indicating whether the go live request has been sent for the room. It is used by mod_visitors_component.lua to manage the go live state of the room.
...
- junbri_throttle - A throttle object used to limit the number of Junbri requests per room. It is created in mod_filter_iq_junbri.lua to prevent abuse of Junbri resources.
- juncto_meet_tenant_mismatch - A boolean value indicating whether there is a tenant mismatch comparing room tenant and jwt tenant value.
- juncto_shared_files - A table containing shared files in the room. The keys are file IDs and the values are file objects with properties like fileId, name, size, type, and url. Used by mod_filesharing_component.lua to manage file sharing in the room.
- junctoMetadata - The metadata for the room. Example values:
```json
    {
...
- pre_join_queue - A queue used to hold join requests when the room is locked waiting for Juncofo to join. It is set by mod_muc_meeting_id.lua when the room is locked. The queue is processed once Juncofo joins the room.
- send_default_permissions_to - A table containing the default permissions to be sent to occupants in the room. The keys are bare JIDs and the values are boolean values indicating whether the permissions should be sent. Used by mod_juncto_permissions.lua to manage permissions in the room.
