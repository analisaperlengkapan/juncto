...
module:hook("muc-room-created", function(event)
    ...
    if room.junctoMetadata then
        ...
        room.junctoMetadata.allownersEnabled = not is_moderated(room.jid);
    end
...
module:hook("muc-occupant-pre-join", function (event)
    ...
    if moderated then
        ...
        if not (room_name == session.juncto_meet_room or session.juncto_meet_room == '*') then
            module:log('debug', 'skip allowners for auth user and non matching room name: %s, jwt room name: %s',
                room_name, session.juncto_meet_room);
            return;
        end

        if session.juncto_meet_domain ~= '*' and subdomain ~= session.juncto_meet_domain then
            module:log('debug', 'skip allowners for auth user and non matching room subdomain: %s, jwt subdomain: %s',
                subdomain, session.juncto_meet_domain);
            return;
        end
    end
...
module:hook('room_has_host', function(event)
    ...
    if not tenant and allowner_issuers and not allowner_issuers:contains(session.juncto_meet_auth_issuer) then
