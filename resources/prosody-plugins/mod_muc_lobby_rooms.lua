...
--
-- VirtualHost "junctomeet.example.com"
-- modules_enabled = {
--     "muc_lobby_rooms"
-- }
-- lobby_muc = "lobby.junctomeet.example.com"
-- main_muc = "conference.junctomeet.example.com"
--
-- Component "lobby.junctomeet.example.com" "muc"
--     storage = "memory"
...
local DISPLAY_NAME_REQUIRED_FEATURE = 'http://juncto.org/protocol/lobbyrooms#displayname_required';
...
function destroy_lobby_room(room, newjid, message)
    ...
    if lobby_muc_service and room and room._data.lobbyroom then
        ...
        if lobby_room_obj then
            ...
            if room.junctoMetadata then
                room.junctoMetadata.lobbyEnabled = false;
...
function process_lobby_muc_loaded(lobby_muc, host_module)
    ...
    host_module:hook('host-disco-info-node', function (event)
        ...
        if node == LOBBY_IDENTITY_TYPE
            and session.juncto_web_query_room then
...
process_host_module(main_muc_component_config, function(host_module, host)
    ...
    host_module:hook('muc-config-submitted', function(event)
        ...
        if members_only then
            ...
            if lobby_created then
                module:fire_event('juncto-lobby-enabled', { room = room; });
                ...
                room.junctoMetadata.lobbyEnabled = true;
        ...
        elseif room._data.lobbyroom then
            ...
            module:fire_event('juncto-lobby-disabled', { room = room; });
    ...
    host_module:hook('muc-occupant-pre-join', function (event)
        ...
        if (not displayName or #displayName == 0) and not room._data.lobby_skip_display_name_check then
            ...
            reply:tag('displayname-required', { xmlns = 'http://juncto.org/junctomeet', lobby = 'true' }):up():up();
        ...
        if not affiliation or affiliation == 'none' then
            ...
            if room._data.lobby_extra_reason then
                reply:tag(room._data.lobby_extra_reason, { xmlns = 'http://juncto.org/junctomeet' }):up();
            end
            reply:tag('lobbyroom', { xmlns = 'http://juncto.org/junctomeet' }):text(room._data.lobbyroom):up():up();

            -- TODO: Drop this tag at some point (when all mobile clients and jungasi are updated), as this violates the rfc
...
function handle_destroy_lobby(event)
    ...
    -- trigger a 104 (config change) status message so UI state is properly updated for existing users (and juncofo)
