...
-- a module that will filter group messages based on features (juncto_meet_context_features)
...
local function on_message(event)
    ...
    if room.junctoMetadata and room.junctoMetadata.permissions
        and room.junctoMetadata.permissions.groupChatRestricted
        and not is_feature_allowed('send-groupchat', session.juncto_meet_context_features) then
...
module:hook('juncto-visitor-groupchat-pre-route', on_message); -- visitors messages
