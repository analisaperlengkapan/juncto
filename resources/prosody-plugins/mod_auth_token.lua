...
module:hook("pre-resource-unbind", function (e)
    ...
    prosody.events.fire_event('juncto-pre-session-unbind', {
...
function provider.get_sasl_handler(session)
    ...
    local function get_username_from_token(self, message)
        ...
        local pre_event_result = prosody.events.fire_event("pre-juncto-authentication-fetch-key", session);
        ...
        if res == false then
            module:log("warn",
                "Error verifying token err:%s, reason:%s tenant:%s room:%s user_agent:%s",
                    error, reason, session.juncto_web_query_prefix, session.juncto_web_query_room,
        ...
        local shouldAllow = prosody.events.fire_event("juncto-access-ban-check", session);
        ...
        local customUsername = prosody.events.fire_event("pre-juncto-authentication", session);
        ...
        local post_event_result = prosody.events.fire_event("post-juncto-authentication", session);
