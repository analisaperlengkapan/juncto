...
local is_sip_jungasi = util.is_sip_jungasi;
...
local is_sip_junbri_join = util.is_sip_junbri_join;
...
local function get_visitors_room_metadata(room)
    ...
    if not room.junctoMetadata.visitors then
        room.junctoMetadata.visitors = {};
    end
    return room.junctoMetadata.visitors;
end
...
function send_json_message(to_jid, json_message)
    local stanza = st.message({ from = module.host; to = to_jid; })
         :tag('json-message', { xmlns = 'http://juncto.org/junctomeet' }):text(json_message):up();
    module:send(stanza);
end
...
local function connect_vnode_received(room, vnode)
    module:context(muc_domain_base):fire_event('juncto-connect-vnode', { room = room; vnode = vnode; });
    ...
    room._connected_vnodes:set(vnode..'.meet.juncto', {});
end

local function disconnect_vnode_received(room, vnode)
    module:context(muc_domain_base):fire_event('juncto-disconnect-vnode', { room = room; vnode = vnode; });

    if not room._connected_vnodes then
        return;
    end
    room._connected_vnodes:set(vnode..'.meet.juncto', nil);
...
local function stanza_handler(event)
    ...
    local visitors_iq = event.stanza:get_child('visitors', 'juncto:visitors');
    ...
    if not room then
        -- this maybe as we receive the iq from juncofo after the room is already destroyed
    ...
    -- connect and disconnect are only received from juncofo
    ...
    -- request to update metadata service for jungasi languages
...
local function go_live(room)
    ...
    if room._juncto_go_live_sent then
        if DEBUG then
    ...
    if room.junctoMetadata and room.junctoMetadata.visitors
            and room.junctoMetadata.visitors.live ~= nil and room.junctoMetadata.visitors.live == false then
        if DEBUG then
    ...
    room._juncto_go_live_sent = true;
...
process_host_module(muc_domain_prefix..'.'..muc_domain_base, function(host_module, host)
    ...
    host_module:hook('muc-occupant-pre-join', function (event)
        ...
        if (auto_allow_promotion or get_visitors_room_metadata(room).autoPromote and (is_live or is_live == nil))
            or ignore_list:contains(jid.host(stanza.attr.from)) -- junbri or other domains to ignore
            or is_sip_jungasi(stanza)
            or is_sip_junbri_join(stanza)
            or table_find(room._data.moderators, session.juncto_meet_context_user and session.juncto_meet_context_user.id)
            or (room._data.moderator_id and room._data.moderator_id == (session.juncto_meet_context_user and session.juncto_meet_context_user.id))
            or (room._data.moderator_id and room._data.moderator_id == session.juncto_meet_context_group)
            or table_find(room._data.participants, session.juncto_meet_context_user and session.juncto_meet_context_user.id) then
            if DEBUG then
        ...
        if visitors_promotion_map[room.jid] then
            ...
            if visitors_promotion_map[room.jid][jid.node(stanza.attr.from)] -- promotion was approved
                or in_ignore_list then -- junbri or other domains to ignore
                -- allow join
            ...
            session.send(st.error_reply(stanza, 'cancel', 'not-allowed', 'Visitor needs to be allowed by a moderator')
                :tag('promotion-not-allowed', { xmlns = 'juncto:visitors' }));
            return true;
        elseif is_vpaas(room) then
            ...
            if session.juncto_meet_context_user and session.juncto_meet_context_user.role == 'visitor' then
                ...
                session.send(st.error_reply(
                    stanza, 'cancel', 'not-allowed', 'Visitor tried to join the main room without approval')
                        :tag('no-main-participants', { xmlns = 'juncto:visitors' }));
                return true;
            end
        elseif room._data.participants then
                ...
                session.send(st.error_reply(
                    ...
                    'Tried to join the main (not live or without main participants) room')
                        :tag('not-live-room', { xmlns = 'juncto:visitors' }));
                return true;
        end

    end, 7); -- after muc_meeting_id, the logic for not joining before juncofo
    ...
    host_module:hook('juncto-endpoint-message-received', function(event)
        ...
        if data.action == "demote-request" then
            ...
            for _, room_occupant in room:each_occupant() do
                -- do not send it to juncofo or back to the sender
    ...
    if visitors_queue_service then
        ...
        host_module:hook('juncto-metadata-updated', function (event)
        ...
        host_module:hook('muc-occupant-joined', function (event)
            ...
            if always_visitors_enabled then
                ...
                room.junctoMetadata.visitorsEnabled = true;
            end
...
prosody.events.add_handler('pre-juncto-authentication', function(session)
    if not session.customusername or not session.juncto_web_query_room then
        return nil;
    end

    local room = get_room_by_name_and_subdomain(session.juncto_web_query_room, session.juncto_web_query_prefix);
    if not room then
        return nil;
    end
...
module:hook_global('juncto-breakout-occupant-leaving', handle_occupant_leaving_breakout);
