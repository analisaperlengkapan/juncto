...
-- Clean up the room in case it is empty and has only junbri and jungasi-transcriber left in the meeting

local util = module:require 'util';
local is_admin = util.is_admin;
local is_transcriber = util.is_transcriber;
local is_junbri = util.is_junbri;
...
module:hook('muc-occupant-joined', function (event)
    ...
    if not is_junbri(occupant.jid) and not is_transcriber(occupant.jid) and room.empty_destroy_timer then
...
module:hook('muc-occupant-left', function (event)
    ...
    if is_admin(occupant.bare_jid) or is_junbri(occupant.jid) or is_transcriber(occupant.jid)
        or room._data.breakout_rooms_active then
        return;
    end

    for _, o in room:each_occupant() do
        if not is_junbri(o.jid) and not is_transcriber(o.jid)
    ...
    -- seems the room only has junbri and transcriber, add a timeout to destroy the room
