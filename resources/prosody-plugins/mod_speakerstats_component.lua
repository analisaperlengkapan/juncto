...
function on_message(event)
    ...
    local speakerStats
        = event.stanza:get_child('speakerstats', 'http://juncto.org/junctomeet');
    ...
    local newFaceLandmarks = event.stanza:get_child('faceLandmarks', 'http://juncto.org/junctomeet');
...
function occupant_joined(event)
    ...
    if is_healthcheck_room(room.jid)
        ...
        or is_transcriber_jungasi(stanza)
        or is_junbri(occupant) then
        return;
    end
    ...
    if room.speakerStats then
        ...
        local context_user = event.origin and event.origin.juncto_meet_context_user or nil;
