...
function init_session(event)
    ...
    if query ~= nil then
        ...
        -- customusername can be used with combination with "pre-juncto-authentication" event to pre-set a known jid to a session
        session.customusername = query and params.customusername or nil;

        -- The room name and optional prefix from the web query
        session.juncto_web_query_room = params.room;
        session.juncto_web_query_prefix = params.prefix or "";
    end
