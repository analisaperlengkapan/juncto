...
--         "muc_breakout_rooms"
--     }
--     breakout_rooms_muc = "breakout.junctomeet.example.com"
--     main_muc = "muc.junctomeet.example.com"
--
-- Component "breakout.junctomeet.example.com" "muc"
--     restrict_room_creation = true
--     storage = "memory"
--     admins = { "focusUser@auth.junctomeet.example.com" }
...
local RENAME_FEATURE = 'http://juncto.org/protocol/breakout_rooms#rename';
...
function send_json_msg(to_jid, json_msg)
    local stanza = st.message({ from = breakout_rooms_muc_component_config; to = to_jid; })
         :tag('json-message', { xmlns = 'http://juncto.org/junctomeet' }):text(json_msg):up();
    module:send(stanza);
end
...
function on_occupant_pre_leave(event)
    ...
    prosody.events.fire_event('juncto-breakout-occupant-leaving', {
...
function process_breakout_rooms_muc_loaded(breakout_rooms_muc, host_module)
    ...
    host_module:hook('host-disco-info-node', function (event)
        ...
        if node == BREAKOUT_ROOMS_IDENTITY_TYPE and session.juncto_web_query_room then
...
module:hook_global('juncto-pre-session-unbind', handle_pre_resource_unbind);
