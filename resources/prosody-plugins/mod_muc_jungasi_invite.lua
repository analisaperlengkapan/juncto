-- A http endpoint to invite jungasi to a meeting via http endpoint
-- jwt is used to validate access
-- Copyright (C) 2023-present Juncto, Inc.
...
-- This module chooses jungasi from the brewery room, so it needs information for the configured brewery
local muc_domain = module:get_option_string("muc_internal_domain_base", 'internal.auth.' .. muc_domain_base);

local jungasi_brewery_room_jid = module:get_option_string("muc_jungasi_brewery_jid", 'jungasibrewery@' .. muc_domain);

local jungasi_bare_jid = module:get_option_string("muc_jungasi_jid", "jungasi@auth." .. muc_domain_base);
local focus_jid = module:get_option_string("muc_juncofo_brewery_jid", jungasi_brewery_room_jid .. "/focus");

local main_muc_service;
local JSON_CONTENT_TYPE = "application/json";

local event_count = module:measure("muc_invite_jungasi_rate", "rate")
local event_count_success = module:measure("muc_invite_jungasi_success", "rate")
...
local function invite_jungasi(conference, phone_no)
    local jungasi_brewery_room = main_muc_service.get_room_from_jid(jungasi_brewery_room_jid);
    if not jungasi_brewery_room then
        module:log("error", "Jungasi brewery room not found")
        return 404, 'Brewery room was not found'
    end
    module:log("info", "Invite jungasi from %s to join conference %s and outbound phone_no %s", jungasi_brewery_room.jid, conference, phone_no)

    --select least stressed Jungasi
    local least_stressed_value = math.huge;
    local least_stressed_jungasi_jid;
    for occupant_jid, occupant in jungasi_brewery_room:each_occupant() do
        local _, _, resource = jid_split(occupant_jid);
        if resource ~= 'focus' then
            local occ = occupant:get_presence();
            local stats_child = occ:get_child("stats", "http://juncto.org/protocol/colibri")

            local is_sip_jungasi = true;
            for stats_tag in stats_child:children() do
                if stats_tag.attr.name == 'supports_sip' and stats_tag.attr.value == 'false' then
                    is_sip_jungasi = false;
                end
            end

            if is_sip_jungasi then
                for stats_tag in stats_child:children() do
                    if stats_tag.attr.name == 'stress_level' then
                        ...
                        if stress_level < least_stressed_value then
                            least_stressed_jungasi_jid = occupant_jid
    ...
    module:log("debug", "Least stressed jungasi selected jid %s value %s", least_stressed_jungasi_jid, least_stressed_value)
    if not least_stressed_jungasi_jid then
        module:log("error", "Cannot invite jungasi from room %s", jungasi_brewery_room.jid)
        return 404, 'Jungasi not found'
    end

    -- invite Jungasi to join the conference
    local _, _, jungasi_res = jid_split(least_stressed_jungasi_jid)
    local jungasi_full_jid = jungasi_bare_jid .. "/" .. jungasi_res;
    local stanza_id = hashes.sha256(random.bytes(8), true);

    local invite_jungasi_stanza = st.iq({ xmlns = "jabber:client", type = "set", to = jungasi_full_jid, from = focus_jid, id = stanza_id })
    ...
    module:log("debug", "Invite jungasi stanza %s", invite_jungasi_stanza)
    jungasi_brewery_room:route_stanza(invite_jungasi_stanza);
    return 200
end
...
local function handle_jungasi_invite(event)
    module:log("debug", "Request for invite jungasi received: reqId %s", event.request.headers["request_id"])
    event_count()
    ...
    --invite jungasi
    local status_code, error_msg = invite_jungasi(conference, phone_no)
...
module:log("info", "Adding http handler for /invite-jungasi on %s", module.host);
module:depends("http");
module:provides("http", {
    default_path = "/";
    route = {
        ["POST invite-jungasi"] = function(event)
            return async_handler_wrapper(event, handle_jungasi_invite)
        end;
    };
});
