...
module:hook("pre-iq/full", function(event)
    ...
    if stanza.name == "iq" then
        ...
        if dial then
            ...
            local feature = dial.attr.to == 'juncto_meet_transcribe' and 'transcription' or 'outbound-call';
            ...
            local user_id, group_id = nil, session.juncto_meet_context_group;
            if session.juncto_meet_context_user then
                user_id = session.juncto_meet_context_user["id"];
            else
                user_id = session.granted_juncto_meet_context_user_id;
                group_id = session.granted_juncto_meet_context_group_id;
            end
            ...
            if session and user_id then
                ...
                if session.juncto_meet_context_group then
                    dial:tag("header", {
                        xmlns = "urn:xmpp:rayo:1",
                        name = OUT_INITIATOR_GROUP_ATTR_NAME,
                        value = tostring(session.juncto_meet_context_group) });
                    dial:up();
                end
            end

            -- we want to instruct jungasi to enter the main room, so send the correct main room jid
            if is_visitor_prosody then
                dial:tag("header", {
                    xmlns = "urn:xmpp:rayo:1",
    ...
    for room in rooms do
        for _, occupant in room:each_occupant() do
            for _, presence in occupant:each_session() do
                ...
                local initiator = is_sip_jungasi(presence);
                ...
                if initiator then
                    ...
                    -- if found a jungasi participant initiated by the concurrent
...
module:hook('juncto-metadata-allow-moderation', function (event)
    ...
    if key == 'recording' and data and data.isTranscribingEnabled ~= nil then
        ...
        -- are present
        if session.juncto_meet_context_features
            and occupant.role ~= 'moderator'
            and is_feature_allowed('transcription', session.juncto_meet_context_features)
            and is_feature_allowed('recording', session.juncto_meet_context_features) then
                local res = {};
                res.isTranscribingEnabled = data.isTranscribingEnabled;
                return res;
        elseif not session.juncto_meet_context_features and occupant.role == 'moderator' then
            return data;
        else
            return nil;
        end
