...
local is_transcriber_jungasi = util.is_transcriber_jungasi;
...
local function send_visitors_iq(conference_service, room, type)
    ...
    local visitors_iq = st.iq({
      ...
      :tag(type, { xmlns = 'juncto:visitors',
    ...
    if type == 'update' then
        visitors_iq:tag('moderators', { xmlns = 'juncto:visitors' });
        ...
        if room.juncto_shared_files then
            visitors_iq:tag('files', { xmlns = 'juncto:visitors' });
            for k, v in pairs(room.juncto_shared_files) do
...
-- an event received from visitors component, which receives iqs from juncofo
local function connect_vnode(event)
    local room, vnode = event.room, event.vnode;
    local conference_service = muc_domain_prefix..'.'..vnode..'.meet.juncto';
...
module:hook('juncto-connect-vnode', connect_vnode);
...
module:hook('juncto-disconnect-vnode', disconnect_vnode);
...
module:hook('presence/full', function(event)
    ...
    if stanza.attr.type == 'unavailable' and from_host ~= main_muc_component_config then
        ...
        if visitors_nodes[room_jid] and visitors_nodes[room_jid].nodes
            ...
            -- we clean only on disconnect coming from juncofo
        end
    end
end, 900);
...
process_host_module(main_muc_component_config, function(host_module, host)
    ...
    host_module:hook('muc-occupant-pre-change', function (event)
        ...
        -- filter focus and configured domains (used for junbri and transcribers)
        if is_admin(stanza.attr.from) or visitors_nodes[room.jid] == nil
            or (ignore_list:contains(jid.host(occupant.bare_jid)) and not is_transcriber_jungasi(stanza)) then
            return;
        end
    ...
    host_module:hook('muc-occupant-left', function (event)
        ...
        -- ignore configured domains (junbri and transcribers)
    ...
    host_module:hook('muc-occupant-joined', function (event)
        ...
        -- filter focus, ignore configured domains (junbri and transcribers)
        if is_admin(stanza.attr.from) or visitors_nodes[room.jid] == nil
            or (ignore_list:contains(jid.host(occupant.bare_jid)) and not is_transcriber_jungasi(stanza)) then
            return;
        end
    ...
    host_module:hook('muc-occupant-groupchat', function(event)
        ...
        -- filter sending messages from transcribers/junbris to visitors
    ...
    host_module:hook('muc-occupant-groupchat', function(event)
        ...
        if host_module:fire_event('juncto-visitor-groupchat-pre-route', event) then
            -- message filtered
            return;
        end
...
module:hook('juncto-lobby-enabled', update_vnodes_for_room);
module:hook('juncto-lobby-disabled', update_vnodes_for_room);
module:hook('juncto-filesharing-updated', update_vnodes_for_room);
