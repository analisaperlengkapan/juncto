...
local ACCESS_MANAGER_URL = module:get_option_string("muc_prosody_juncto_access_manager_url");
if not ACCESS_MANAGER_URL then
    module:log("warn", "No 'muc_prosody_juncto_access_manager_url' option set, disabling module");
    return
end
            ...
            if code == 200 then
                ...
                if r['access'] ~= nil and r['access'] == false then
                    module:log("info", "User is banned room:%s tenant:%s user_id:%s group:%s",
                        session.juncto_meet_room, session.juncto_web_query_prefix,
                        inspect(session.juncto_meet_context_user), session.juncto_meet_context_group);
...
prosody.events.add_handler("juncto-access-ban-check", function(session)
