...
-- This will prevent anyone joining the call till juncofo and one moderator join the room
...
module:hook('muc-occupant-pre-join', function (event)
    ...
    -- we ignore juncofo as we want it to join the room or if the room has already seen its
    -- authenticated host
    ...
    if not room.has_host then
        if module:fire_event('room_has_host', { room = room; occupant = occupant; session = session; }) then
            ...
            audit_logger('room_jid:%s created_by:%s', room.jid,
                session.juncto_meet_context_user and session.juncto_meet_context_user.id or 'nil');
