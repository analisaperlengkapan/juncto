...
local function limit_bytes_in(bytes, session)
	local sess_throttle = session.juncto_throttle;
...
local function throttle_session(session, rate, timeout)
    if not session.juncto_throttle then
        if (session.conn and session.conn.setlimit) then
            session.juncto_throttle_counter = session.juncto_throttle_counter + 1;
            module:log("info", "Enabling throttle (%s bytes/s) via setlimit, session=%s, ip=%s, counter=%s.",
                rate, session.id, session.ip, session.juncto_throttle_counter);
            session.conn:setlimit(rate);
            if timeout then
                if session.juncto_throttle_timer then
                    -- if there was a timer stop it as we will schedule a new one
                    session.juncto_throttle_timer:stop();
                    session.juncto_throttle_timer = nil;
                end
                session.juncto_throttle_timer = module:add_timer(timeout, function()
...
local function filter_hook(session)
	...
	session.juncto_throttle_counter = 0;
