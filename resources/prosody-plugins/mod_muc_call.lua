...
--     https://meet.juncto.net/juncto/ProductiveMeeting
--     https://meet.juncto.net/MoreProductiveMeeting
...
module:hook(
	...
	--   not (x == y)' can be replaced by 'x ~= y' (if neither side is a table or NaN)
        ...
        local invite = function()
            ...
            module:fire_event('juncto-call-invite', { stanza = event.stanza; url = url; call_id = call_id; });
        end

        local cancel = function()
            local url = assert(url_from_room_jid(event.stanza.attr.from))
            local status = event.stanza:get_child_text("status")
            module:fire_event('juncto-call-cancel', {
                stanza = event.stanza;
                url = url;
        ...
        local missed = function()
            cancel()
            module:fire_event('juncto-call-missed', { stanza = event.stanza; call_id = call_id; });
        end
