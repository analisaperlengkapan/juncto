...
import { JunctoTrackEvents } from '../../../react/features/base/lib-juncto';
...
export default class LargeVideoManager {
    ...
    destroy() {
        ...
        if (this.videoTrack && !this.videoTrack.local) {
            this.videoTrack.junctoTrack.off(JunctoTrackEvents.TRACK_STREAMING_STATUS_CHANGED,
                this.handleTrackStreamingStatusChanged);
            APP.store.dispatch(trackStreamingStatusChanged(this.videoTrack.junctoTrack,
                this.videoTrack.junctoTrack.getTrackStreamingStatus()));
        }
    ...
    scheduleLargeVideoUpdate() {
        ...
        preUpdate.then(() => {
            ...
            // TODO: when this class is converted to a function react component,
            // use a custom hook to update a local track streaming status.
            if (this.videoTrack?.junctoTrack?.getSourceName() !== videoTrack?.junctoTrack?.getSourceName()
                || this.videoTrack?.junctoTrack?.isP2P !== videoTrack?.junctoTrack?.isP2P) {
            // In the case where we switch from junctobridge to p2p when we need to switch the p2p and junctobridge track, they will be
            ...
            // the video because of the old track inactive streaming status.
                ...
                if (this.videoTrack && !this.videoTrack.local) {
                    this.videoTrack.junctoTrack.on(JunctoTrackEvents.TRACK_STREAMING_STATUS_CHANGED,
                        this.handleTrackStreamingStatusChanged);
                    APP.store.dispatch(trackStreamingStatusChanged(this.videoTrack.junctoTrack,
                        this.videoTrack.junctoTrack.getTrackStreamingStatus()));
                }
            ...
            logger.debug(`scheduleLargeVideoUpdate: Remote track ${videoTrack?.junctoTrack}, isVideoMuted=${
     ...
     *
     * @param {JunctoTrack} junctoTrack the track with streaming status updated
     * @param {JunctoTrackStreamingStatus} streamingStatus the updated track streaming status
     *
     * @private
     */
    handleTrackStreamingStatusChanged(junctoTrack, streamingStatus) {
        APP.store.dispatch(trackStreamingStatusChanged(junctoTrack, streamingStatus));
    }
     ...
     * @param {JunctoTrack?} stream new stream
