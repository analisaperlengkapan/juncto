...
import JunctoMeetJS, { JunctoRecordingConstants } from '../../react/features/base/lib-juncto';
...
import { getJunctoMeetTransport } from '../transport';
 ...
 * @type {Transport}
 */
const transport = getJunctoMeetTransport();
...
function initCommands() {
    commands = {
        ...
        'set-blurred-background': blurType => {
            ...
            const videoTrack = getLocalVideoTrack(tracks)?.junctoTrack;
         ...
         * Only applies to certain juncto meet deploys.
        ...
        'set-virtual-background': (enabled, backgroundImage) => {
            const tracks = APP.store.getState()['features/base/tracks'];
            const junctoTrack = getLocalVideoTrack(tracks)?.junctoTrack;
    ...
    transport.on('request', (request, callback) => {
        ...
        case 'get-livestream-url': {
            ...
            if (conference) {
                const activeSession = getActiveSession(state, JunctoRecordingConstants.mode.STREAM);
...
function shouldBeEnabled() {
    return (
        typeof API_ID === 'number'
            ...
            // the location/URL because then it is very likely that the Juncto
...
function toggleScreenSharing(enable) {
    if (JunctoMeetJS.isDesktopSharingEnabled()) {
 ...
 * interface to access Juncto features by external applications that embed
 * Juncto.
 */
class API {
    _enabled;
     ...
     * information from external applications that embed Juncto. It also
    ...
    /**
     * Notifies the external application (spot) that the local juncto-participant
     * has a status update.
     *
    ...
    */
     ...
     * @param {string} formattedDisplayName - The display name shown in Juncto
     ...
     * problem establishing a link between the app and the JUNCTOBRIDGE server or the remote peer in case of P2P.
     ...
     * the JUNCTOBRIDGE server, or the other peer in case of P2P.
     *
     * @returns {void}
     */
