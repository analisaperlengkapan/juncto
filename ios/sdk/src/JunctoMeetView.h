...
#import "JunctoMeetConferenceOptions.h"
#import "JunctoMeetViewDelegate.h"
...
@interface JunctoMeetView : UIView

@property (nonatomic, nullable, weak) id<JunctoMeetViewDelegate> delegate;

/**
 * Joins the conference specified by the given options. The given options will
 * be merged with the defaultConferenceOptions (if set) in JunctoMeet. If there
 * is an already active conference it will be automatically left prior to
 * joining the new one.
 */
- (void)join:(JunctoMeetConferenceOptions *_Nullable)options;
/**
