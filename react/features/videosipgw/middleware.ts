...
MiddlewareRegistry.register(({ dispatch }) => next => action => {
    ...
    case CONFERENCE_JOIN_IN_PROGRESS: {
        ...
        conference.on(
            JunctoConferenceEvents.VIDEO_SIP_GW_AVAILABILITY_CHANGED,
            (status: string) => dispatch(_availabilityChanged(status)));
        conference.on(
            JunctoConferenceEvents.VIDEO_SIP_GW_SESSION_STATE_CHANGED,
 ...
 * @param {Object} conference - The JunctoConference to invite the rooms to.
 * @param {Function} dispatch - The redux dispatch function for emitting state
 * changes (queuing error notifications).
 * @private
 * @returns {void}
 */
function _inviteRooms(rooms: ISipRoom[], conference: IJunctoConference, dispatch: IStore['dispatch']) {
    for (const room of rooms) {
        ...
        if (sipAddress && displayName) {
            ...
            if (newSession instanceof Error) {
                ...
                switch (e.message) {
                case JunctoSIPVideoGWStatus.ERROR_NO_CONNECTION: {
                ...
                case JunctoSIPVideoGWStatus.ERROR_SESSION_EXISTS: {
...
function _sessionStateChanged(
    ...
    case JunctoSIPVideoGWStatus.STATE_PENDING: {
    ...
    case JunctoSIPVideoGWStatus.STATE_FAILED: {
    ...
    case JunctoSIPVideoGWStatus.STATE_OFF: {
        if (event.failureReason === JunctoSIPVideoGWStatus.STATUS_BUSY) {
            return showErrorNotification({
