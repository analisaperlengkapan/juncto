...
import { isAnalyticsEnabled } from '../base/lib-juncto/functions.any';
...
/**
 * Sends an event through the lib-juncto AnalyticsAdapter interface.
 *
 * @param {Object} event - The event to send. It should be formatted as
 * described in AnalyticsAdapter.js in lib-juncto.
 * @returns {void}
 */
...
export async function createHandlers({ getState }: IStore) {
    getJunctoMeetGlobalNS().analyticsHandlers = [];
...
/**
 * Inits JunctoMeetJS.analytics by setting permanent properties and setting the handlers from the loaded scripts.
 * NOTE: Has to be used after JunctoMeetJS.init. Otherwise analytics will be null.
 *
 * @param {Store} store - The redux store in which the specified {@code action} is being dispatched.
...
function _loadHandlers(scriptURLs: string[] = [], handlerConstructorOptions: Object) {
    ...
    return Promise.all(promises).then(values => {
        ...
        for (const Handler of getJunctoMeetGlobalNS().analyticsHandlers) {
