...
export function fixDeviceID(amplitude: Types.BrowserClient) {
    const deviceId = junctoLocalStorage.getItem(DEVICE_ID_KEY);
    ...
    if (deviceId) {
        ...
        if (newDeviceId) {
            junctoLocalStorage.setItem(DEVICE_ID_KEY, JSON.stringify(newDeviceId));
        }
    }
}
...
export function getDeviceID() {
    return junctoLocalStorage.getItem(DEVICE_ID_KEY);
}
