...
export function invite(
    ...
    return (
        ...
        if (typeof conference === 'undefined') {
            // Only keep invitees which can get an invite request from Juncto UI
            const junctoInvitees = invitees.filter(({ type }) => type !== INVITE_TYPES.EMAIL);

            // Invite will fail before CONFERENCE_JOIN. The request will be
            // cached in order to be executed on CONFERENCE_JOIN.
            if (junctoInvitees.length) {
                return new Promise(resolve => {
                    dispatch(addPendingInviteRequest({
                        invitees: junctoInvitees,
