...
export function getInviteResultsForQuery(
    ...
    // which may be valid for SIP (jungasi) calls. If the dialOutAuthUrl is
...
export function getInviteText({
    ...
    const dialIn = state['features/invite'];
    const inviteUrl = getInviteURL(state);
    const currentLiveStreamingSession = getActiveSession(state, JunctoRecordingConstants.mode.STREAM);
...
export function invitePeopleAndChatRooms(
    ...
    if (!token && peopleSearchTokenLocation) {
        token = junctoLocalStorage.getItem(peopleSearchTokenLocation) ?? '';
    }
