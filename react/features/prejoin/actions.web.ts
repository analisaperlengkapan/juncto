...
import { createLocalTrack } from '../base/lib-juncto/functions';
...
export function joinConferenceWithoutAudio() {
    return async function(dispatch: IStore['dispatch'], getState: IStore['getState']) {
        ...
        const audioTrack = getLocalAudioTrack(tracks)?.junctoTrack;
...
export function replaceAudioTrackById(deviceId: string) {
    return async (dispatch: IStore['dispatch'], getState: IStore['getState']) => {
        try {
            ...
            const oldTrack = getLocalAudioTrack(tracks)?.junctoTrack;
...
export function replaceVideoTrackById(deviceId: string) {
    return async (dispatch: IStore['dispatch'], getState: IStore['getState']) => {
        try {
            ...
            const oldTrack = getLocalVideoTrack(tracks)?.junctoTrack;
 ...
 * is triggered as a result of call migration initiated by Juncofo.
 *
