...
StateListenerRegistry.register(
    ...
    (conference, { dispatch, getState }, previousConference) => {
        if (conference && !previousConference) {
            ...
            conference.on(JunctoConferenceEvents.AV_MODERATION_APPROVED, ({ mediaType }: { mediaType: MediaType; }) => {
            ...
            conference.on(JunctoConferenceEvents.AV_MODERATION_REJECTED, ({ mediaType }: { mediaType: MediaType; }) => {
                dispatch(localParticipantRejected(mediaType));
            });

            conference.on(JunctoConferenceEvents.AV_MODERATION_CHANGED, ({ enabled, mediaType, actor }: {
            ...
            conference.on(
                JunctoConferenceEvents.AV_MODERATION_PARTICIPANT_APPROVED,
            ...
            // this is received by moderators
            conference.on(
                JunctoConferenceEvents.AV_MODERATION_PARTICIPANT_REJECTED,
                ({ participant, mediaType }: { mediaType: MediaType; participant: { _id: string; }; }) => {
                    const { _id: id } = participant;
