...
class VirtualBackgroundPreview extends PureComponent<IProps, IState> {
    ...
    /**
     * Destroys the junctoTrack object.
     *
     * @param {Object} junctoTrack - The track that needs to be disposed.
     * @returns {Promise<void>}
     */
    _stopStream(junctoTrack: any) {
        if (junctoTrack) {
            junctoTrack.dispose();
        }
    }
    ...
    async _setTracks() {
        try {
            ...
            const [ junctoTrack ] = await createLocalTracksF({
    ...
    async _applyBackgroundEffect() {
        ...
        await this.props.dispatch(toggleBackgroundEffect(this.props.options, this.state.junctoTrack));
        this.props.loadedPreview(true);
    ...
    override componentWillUnmount() {
        ...
        this._stopStream(this.state.junctoTrack);
    }
     ...
     */
    override render() {
        const { junctoTrack } = this.state;
        const classes = withStyles.getClasses(this.props);

        return (
            <div className = { classes.virtualBackgroundPreview }>
                {junctoTrack
                    ? this._renderPreviewEntry(junctoTrack)
