 ...
 * @param {Object} junctoTrack - Represents the juncto track that will have backgraund effect applied.
 * @returns {Promise}
 */
export function toggleBackgroundEffect(options: IVirtualBackground, junctoTrack: any) {
        ...
        if (junctoTrack) {
            try {
                if (options.backgroundEffectEnabled) {
                    await junctoTrack.setEffect(await createVirtualBackgroundEffect(virtualBackground, dispatch));
                } else {
                    await junctoTrack.setEffect(undefined);
 ...
 *
 * @param {JunctoLocalTrack} videoTrack - The targeted video track.
