...
import { JunctoConferenceEvents, JunctoConnectionErrors } from '../base/lib-juncto';
...
MiddlewareRegistry.register(({ dispatch, getState }) => next => action => {
    ...
    case CONFERENCE_JOINED: {
        ...
        if (getState()['features/visitors'].iAmVisitor) {
            ...
            conference.on(JunctoConferenceEvents.VISITORS_SUPPORTED_CHANGED, (value: boolean) => {
                dispatch(setVisitorsSupported(value));
            });
        }

        conference.on(JunctoConferenceEvents.VISITORS_MESSAGE, (
        ...
        conference.on(JunctoConferenceEvents.VISITORS_REJECTION, () => {
    ...
    case CONNECTION_FAILED: {
        ...
        if (error?.name !== JunctoConnectionErrors.NOT_LIVE_ERROR) {
            break;
        }
