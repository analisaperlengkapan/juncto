...
/**
 * Notifies this instance about a "Follow Me" command received by the Juncto
 * conference.
 *
...
function _onFollowMeCommand(attributes: any = {}, id: string, store: IStore) {
    ...
    if (participantSendingCommand) {
        ...
        // This is the case of junbri receiving commands from a hidden participant.
        const { iAmRecorder } = state['features/base/config'];
        const { conference } = state['features/base/conference'];

        // As this participant is not stored in redux store we do the checks on the JunctoParticipant from lib-juncto
