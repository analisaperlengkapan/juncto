...
/**
 * Registers listener for {@link JunctoConferenceEvents.MESSAGE_RECEIVED} that
 * will perform various chat related activities.
 *
 * @param {JunctoConference} conference - The conference instance on which the
 * new event listener will be registered.
 * @param {Object} store - The redux store object.
 * @private
 * @returns {void}
 */
function _addChatMsgListener(conference: IJunctoConference, store: IStore) {
    ...
    conference.on(
        JunctoConferenceEvents.REACTION_RECEIVED,
    ...
    conference.on(
        JunctoConferenceEvents.CONFERENCE_ERROR, (errorType: string, error: Error) => {
            errorType === JunctoConferenceErrors.CHAT_ERROR && _handleChatError(store, error);
        });
}
