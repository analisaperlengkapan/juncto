...
function _conferenceWillLeave({ dispatch, getState }: IStore, next: Function, action: AnyAction) {
    ...
    if (!doNotStoreRoom && !isEmbedded()) {
        ...
        /**
         * FIXME:
         * It is better to use action.conference[JUNCTO_CONFERENCE_URL_KEY]
         ...
         * JUNCTO_CONFERENCE_URL_KEY so we cannot call it and must use the other way.
         */
        if (typeof APP === 'undefined') {
            const { conference } = action;

            // eslint-disable-next-line @typescript-eslint/prefer-optional-chain
            locationURL = conference && conference[JUNCTO_CONFERENCE_URL_KEY];
        } else {
