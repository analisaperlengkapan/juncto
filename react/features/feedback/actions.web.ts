...
import { sendFeedbackToJunctoServiceRequest, shouldSendJunctoServiceFeedbackMetadata } from './functions.web';
 ...
 */
export function maybeOpenFeedbackDialog(conference: IJunctoConference, title?: string) {
    type R = {
    ...
    return (dispatch: IStore['dispatch'], getState: IStore['getState']): Promise<R> => {
        ...
        } else if (shouldSendJunctoServiceFeedbackMetadata(state)
 ...
 *
 * @param {JunctoConference} conference - The JunctoConference that is being
 ...
 * @returns {Object}
 */
export function openFeedbackDialog(conference?: IJunctoConference, title?: string, onClose?: Function) {
...
/**
 * Sends feedback metadata to JunctoService endpoint.
 *
 * @param {JunctoConference} conference - The JunctoConference that is being rated.
 * @param {Object} feedback - The feedback message and score.
 *
 * @returns {Promise}
 */
export function sendJaasFeedbackMetadata(conference: IJunctoConference, feedback: Object) {
    return (_dispatch: IStore['dispatch'], getState: IStore['getState']) => {
        const state = getState();

        if (!shouldSendJunctoServiceFeedbackMetadata(state)) {
            return Promise.resolve();
        }
        ...
        return sendFeedbackToJunctoServiceRequest(jaasFeedbackMetadataURL, feedbackData);
    };
}
 ...
 * rating.
 * @param {JunctoConference} conference - The JunctoConference for which the
