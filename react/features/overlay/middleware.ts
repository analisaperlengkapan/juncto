...
import { JunctoConferenceErrors, JunctoConnectionErrors } from '../base/lib-juncto';
import {
    isFatalJunctoConferenceError,
    isFatalJunctoConnectionError
} from '../base/lib-juncto/functions.any';
...
const RN_NO_RELOAD_DIALOG_ERRORS = [
    JunctoConnectionErrors.NOT_LIVE_ERROR,
    JunctoConnectionErrors.SHARD_CHANGED_ERROR,
    JunctoConferenceErrors.CONFERENCE_ACCESS_DENIED,
    JunctoConferenceErrors.CONFERENCE_DESTROYED,
    JunctoConferenceErrors.CONNECTION_ERROR,
    JunctoConferenceErrors.CONFERENCE_RESTARTED
];
...
const getErrorExtraInfo = (state: any, error: ErrorType) => {
    ...
    if (error === conferenceError) {
        return {
            ...
            isFatal: isFatalJunctoConferenceError(error.name || error)
        };
    }
    ...
    if (error === connectionError) {
        return {
            ...
            isFatal: isFatalJunctoConnectionError(error.name || error)
        };
    }
};
