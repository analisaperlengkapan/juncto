...
function _syncTrackState({ getState }: IStore, next: Function, action: AnyAction) {
    ...
    if (junctoTrack.isLocal() && conference?.callUUID) {
        switch (junctoTrack.getType()) {
        case 'audio': {
            _updateCallIntegrationMuted(conference, state);
            break;
        }
 ...
 * @returns {void}
 */
function _updateCallIntegrationMuted(conference: IJunctoConference, state: IReduxState) {
