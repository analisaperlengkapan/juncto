...
import { JUNCTO_CONFERENCE_URL_KEY } from '../../base/conference/constants';
import {
    forEachConference,
    getCurrentConference,
    isRoomValid
} from '../../base/conference/functions';
import { IJunctoConference } from '../../base/conference/reducer';
...
import {
    JUNCTO_CONNECTION_CONFERENCE_KEY,
    JUNCTO_CONNECTION_URL_KEY
} from '../../base/connection/constants';
import { getURLWithoutParams } from '../../base/connection/utils';
import { JunctoConferenceEvents, JunctoRecordingConstants } from '../../base/lib-juncto';
...
externalAPIEnabled && MiddlewareRegistry.register(store => next => action => {
    ...
    case CONFERENCE_FAILED: {
        ...
        // JunctoConferenceErrors.PASSWORD_REQUIRED and will retry joining the
    ...
    case CONNECTION_DISCONNECTED: {
        // FIXME: This is a hack. See the description in the JUNCTO_CONNECTION_CONFERENCE_KEY constant definition.
        // Check if this connection was attached to any conference.
        // If it wasn't, fake a CONFERENCE_TERMINATED event.
        const { connection } = action;
        const conference = connection[JUNCTO_CONNECTION_CONFERENCE_KEY];

        if (!conference) {
            // This action will arrive late, so the locationURL stored on the state is no longer valid.
            const locationURL = connection[JUNCTO_CONNECTION_URL_KEY];
...
function _registerForNativeEvents(store: IStore) {
    ...
    eventEmitter.addListener(ExternalAPI.START_RECORDING, (
        ...
        if (mode === JunctoRecordingConstants.mode.STREAM && !(youtubeStreamKey || rtmpStreamKey)) {
        ...
        if (mode === JunctoRecordingConstants.mode.FILE) {
            ...
            if (dropboxToken) {
                recordingConfig = {
                    mode: JunctoRecordingConstants.mode.FILE,
        ...
        } else if (mode === JunctoRecordingConstants.mode.STREAM) {
            recordingConfig = {
                broadcastId: youtubeBroadcastID || rtmpBroadcastID,
                mode: JunctoRecordingConstants.mode.STREAM,
    ...
    eventEmitter.addListener(ExternalAPI.STOP_RECORDING, ({ mode, transcription }: any) => {
        ...
        if (![ JunctoRecordingConstants.mode.FILE, JunctoRecordingConstants.mode.STREAM ].includes(mode)) {
...
function _registerForEndpointTextMessages(store: IStore) {
    ...
    conference?.on(
        JunctoConferenceEvents.MESSAGE_RECEIVED,
    ...
    conference?.on(
        JunctoConferenceEvents.PRIVATE_MESSAGE_RECEIVED,
        (id: string, message: string, timestamp: number) => {
            sendEvent(
                    store,
                    CHAT_MESSAGE_RECEIVED,
...
function _toErrorString(
    ...
    // XXX In lib-juncto and juncto we utilize errors in the form of
 ...
 * {@link CONFERENCE_WILL_JOIN}. That is fine for the Juncto app, because
...
function _sendConferenceEvent(
    ...
    // For these (redux) actions, conference identifies a JunctoConference
    // instance. The external API cannot transport such an object so we have to
    // transport an "equivalent".
    if (conference) { // @ts-ignore
        data.url = _normalizeUrl(conference[JUNCTO_CONFERENCE_URL_KEY]);
...
function _swallowConferenceLeft({ getState }: IStore, action: AnyAction, { url }: { url: string; }) {
    // XXX Internally, we work with JunctoConference instances. Externally
    ...
    // externally for a URL string which identifies a JunctoConference that the
