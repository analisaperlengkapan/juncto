import JunctoMeetJS, { RTCStatsEvents } from '../base/lib-juncto';
...
class RTCStats {
    ...
    sendLogs(logEntries: Array<string | any>) {
        JunctoMeetJS.rtcstats.sendStatsEntry('logs', logEntries);
    }
    ...
    sendE2ERTTData(e2eRttData: E2ERTTData) {
        JunctoMeetJS.rtcstats.sendStatsEntry('e2eRtt', e2eRttData);
    }
    ...
    sendIdentityData(identityData: Object) {
        JunctoMeetJS.rtcstats.sendIdentityEntry(identityData);
    }
    ...
    sendConferenceTimestamp(timestamp: number) {
        JunctoMeetJS.rtcstats.sendStatsEntry('conferenceStartTimestamp', timestamp);
    }
    ...
    sendFaceLandmarksData(faceLandmarksData: FaceLandmarksData) {
        JunctoMeetJS.rtcstats.sendStatsEntry('faceLandmarks', faceLandmarksData);
    }
    ...
    handleRTCStatsEvent(event: any) {
        ...
        case PC_CON_STATE_CHANGE: {
            ...
            // connection and not necessarily with reaching the JUNCTOBRIDGE (due to a firewall or other reasons).
            if (state === PC_STATE_FAILED) {
                const connectionType = isP2P ? 'P2P' : 'JUNCTOBRIDGE';
