...
/**
 * Middleware which intercepts lib-juncto initialization and conference join in order init the
 * rtcstats-client.
 *
...
MiddlewareRegistry.register((store: IStore) => (next: Function) => (action: AnyAction) => {
    ...
    case TRACK_UPDATED: {
        if (isRTCStatsEnabled(state)) {
            const { junctoTrack, muted } = action?.track || { };
            const { isLocal, videoType: trackVideoType, conference } = junctoTrack || { };
