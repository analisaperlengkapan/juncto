...
import { getLocalJunctoAudioTrackSettings } from '../base/tracks/functions.any';
...
export function getAudioDeviceSelectionDialogProps(stateful: IStateful, isDisplayedOnWelcomePage: boolean) {
    ...
    const inputDeviceChangeSupported = JunctoMeetJS.mediaDevices.isDeviceChangeAvailable('input');
    const speakerChangeSupported = JunctoMeetJS.mediaDevices.isDeviceChangeAvailable('output');
    ...
    const audioSettings = state['features/settings'].audioSettings ?? getLocalJunctoAudioTrackSettings(state);
    ...
    let disableAudioInputChange
        = !JunctoMeetJS.mediaDevices.isMultipleAudioInputSupported() && !(disablePreviews && inputDeviceChangeSupported);
    ...
    return {
        ...
        disableDeviceChange: !JunctoMeetJS.mediaDevices.isDeviceChangeAvailable(),
        hasAudioPermission: permissions.audio,
        hideAudioInputPreview: disableAudioInputChange || !JunctoMeetJS.isCollectingLocalStats() || disablePreviews,
...
export function processExternalDeviceRequest( // eslint-disable-line max-params
    ...
    case 'isDeviceChangeAvailable':
        responseCallback(
            JunctoMeetJS.mediaDevices.isDeviceChangeAvailable(
                request.deviceType));
        break;
    case 'isMultipleAudioInputSupported':
        responseCallback(JunctoMeetJS.isMultipleAudioInputSupported());
        break;
