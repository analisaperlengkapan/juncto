...
const statsEmitter = {
     ...
     * conference.
     *
     * @param {JunctoConference} conference - The conference for which
     * {@code statsEmitter} should subscribe for stat updates.
     * @returns {void}
     */
    startListeningForStats(conference: IJunctoConference) {
        conference.on(JunctoConnectionQualityEvents.LOCAL_STATS_UPDATED,
            (stats: IStats) => this._onStatsUpdated(conference.myUserId(), stats));

        conference.on(JunctoConnectionQualityEvents.REMOTE_STATS_UPDATED,
    ...
    _onStatsUpdated(localUserId: string, stats: IStats) {
        ...
        // be primitives, not maps, so here we override the 'lib-juncto'
        // stats objects.
    ...
    _updateLocalCodecs(codecs: any) {
        if (typeof APP !== 'undefined') {
            ...
            for (const track of localTracks) {
                const ssrc = track.junctoTrack?.getSsrc();
                ...
                if (ssrc && Object.keys(codecs).find(key => Number(key) === ssrc)) {
                    ...
                    if (track.codec !== codec) {
                        APP.store.dispatch(trackCodecChanged(track.junctoTrack, codec));
                    }
                }
            }
        }
