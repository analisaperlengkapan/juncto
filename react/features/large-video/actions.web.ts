...
export function captureLargeVideoScreenshot() {
    return (dispatch: IStore['dispatch'], getState: IStore['getState']) => {
        ...
        // Participants that join the call video muted do not have a junctoTrack attached.
        if (!participantTrack?.junctoTrack) {
            return promise;
        }
        const videoStream = participantTrack.junctoTrack.getOriginalStream();
