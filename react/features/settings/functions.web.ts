 ...
 * all the video junctoTracks and appropriate errors for the given device ids.
 *
...
export function createLocalAudioTracks(devices: Array<{ deviceId: string; label: string; }>, timeout?: number) {
    return Promise.all(
        devices.map(async ({ deviceId, label }) => {
            let junctoTrack = null;
            let hasError = false;

            try {
                junctoTrack = await createLocalTrack('audio', deviceId, timeout);
            } catch (err) {
                hasError = true;
            }
