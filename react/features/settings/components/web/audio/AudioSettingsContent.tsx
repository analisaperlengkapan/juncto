...
const browser = JunctoMeetJS.util.browser;
...
const AudioSettingsContent = ({
     ...
     *
     * @param {Object} data - An object with the deviceId, junctoTrack & label of the microphone.
     * @param {number} index - The index of the element, used for creating a key.
     * @param {length} length - The length of the microphone list.
     * @returns {React$Node}
     */
    const _renderMicrophoneEntry = (data: { deviceId: string; hasError: boolean; junctoTrack: any; label: string; },
            index: number, length: number) => {
        const { deviceId, junctoTrack, hasError } = data;
     ...
     * @returns {void}
     */
    const _disposeTracks = (tracks: Array<{ junctoTrack: any; }>) => {
        tracks.forEach(({ junctoTrack }) => {
            junctoTrack?.dispose();
        });
    };
