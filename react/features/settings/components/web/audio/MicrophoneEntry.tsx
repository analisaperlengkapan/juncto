...
const MicrophoneEntry = ({
    ...
    const activeTrackRef = useRef(junctoTrack);
     ...
     *
     * @param {number} num - The audio level provided by the junctoTrack.
     * @returns {void}
     */
    ...
    /**
     * Subscribes to audio level changes coming from the junctoTrack.
     *
     * @returns {void}
     */
    const startListening = () => {
        junctoTrack && measureAudioLevels && junctoTrack.on(
            JunctoTrackEvents.TRACK_AUDIO_LEVEL_CHANGED,
            updateLevel);
    };

    /**
     * Unsubscribes from changes coming from the junctoTrack.
     *
     * @param {Object} track - The junctoTrack to unsubscribe from.
     * @returns {void}
     */
    const stopListening = (track?: any) => {
        track?.off(JunctoTrackEvents.TRACK_AUDIO_LEVEL_CHANGED, updateLevel);
        setLevel(-1);
    };
    ...
    useEffect(() => {
        ...
        startListening();
        activeTrackRef.current = junctoTrack;
    }, [ junctoTrack ]);
    ...
    return (
        <li
            ...
            {Boolean(junctoTrack) && measureAudioLevels && <Meter
