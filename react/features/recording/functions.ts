...
import { JunctoRecordingConstants } from '../base/lib-juncto';
...
export function getActiveSession(state: IReduxState, mode: string) {
    ...
    const { status: statusConstants } = JunctoRecordingConstants;
...
export function getSessionStatusToShow(state: IReduxState, mode: string): string | undefined {
    ...
    if (!status && mode === JunctoRecordingConstants.mode.FILE
            && (LocalRecordingManager.isRecordingLocally() || isRemoteParticipantRecordingLocally(state))) {
        status = JunctoRecordingConstants.status.ON;
    }
...
export function isCloudRecordingRunning(state: IReduxState) {
    return Boolean(getActiveSession(state, JunctoRecordingConstants.mode.FILE));
}
...
export function isLiveStreamingRunning(state: IReduxState) {
    return Boolean(getActiveSession(state, JunctoRecordingConstants.mode.STREAM));
}
...
export function shouldRequireRecordingConsent(recorderSession: any, state: IReduxState) {
    ...
    // lib-juncto may set a JunctoParticipant as the initiator of the recording session or the
    ...
    if (!initiatorId || recorderSession.getStatus() === JunctoRecordingConstants.status.OFF) {
        return false;
    }
