...
import JunctoMeetJS, {
    ...
    JunctoRecordingConstants
} from '../base/lib-juncto';
import {
    setAudioMuted,
...
MiddlewareRegistry.register(({ dispatch, getState }) => next => action => {
    ...
    case CONFERENCE_JOIN_IN_PROGRESS: {
        ...
        conference.on(
            JunctoConferenceEvents.RECORDER_STATE_CHANGED,
            (recorderSession: any) => {
                if (recorderSession) {
    ...
    case RECORDING_SESSION_UPDATED: {
        ...
        const { PENDING, OFF, ON } = JunctoRecordingConstants.status;
        ...
        if (updatedSessionData?.status === ON) {
            ...
            // We receive 2 updates of the session status ON. The first one is from junbri when it joins.
            // The second one is from juncofo which will deliver the initiator value. Since the start
            // recording notification uses the initiator value we skip the junbri update and show the
            // notification on the update from juncofo.
            ...
            if (oldSessionData?.status !== ON) {
                ...
                if (mode === JunctoRecordingConstants.mode.FILE && !isRecorderTranscriptionsRunning(state)) {
    ...
    case TRACK_ADDED: {
        ...
        if (LocalRecordingManager.isRecordingLocally()
            ...
            const audioTrack = track.junctoTrack.track;
...
function _showRecordingErrorNotification(session: any, dispatch: IStore['dispatch'], getState: IStore['getState']) {
    ...
    const isStreamMode = mode === JunctoMeetJS.constants.recording.mode.STREAM;

    switch (error) {
    case JunctoMeetJS.constants.recording.error.SERVICE_UNAVAILABLE:
        ...
        break;
    case JunctoMeetJS.constants.recording.error.RESOURCE_CONSTRAINT:
        dispatch(showRecordingError({
        ...
        break;
    case JunctoMeetJS.constants.recording.error.UNEXPECTED_REQUEST:
        ...
        break;
    case JunctoMeetJS.constants.recording.error.POLICY_VIOLATION:
        dispatch(showRecordingWarning({
