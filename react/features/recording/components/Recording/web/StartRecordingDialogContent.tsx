...
class StartRecordingDialogContent extends AbstractStartRecordingDialogContent {
    ...
    _renderAdvancedOptions() {
        ...
        if (selectedRecordingService !== RECORDING_TYPES.JUNCTO_REC_SERVICE || !this._canStartTranscribing()) {
            return null;
        }
    ...
    _renderNoIntegrationsContent() {
        ...
        const switchContent
            = integrationsEnabled || _localRecordingAvailable
                ? (
                    <Switch
                        checked = { selectedRecordingService === RECORDING_TYPES.JUNCTO_REC_SERVICE }
                        className = 'recording-switch'
                        disabled = { isValidating || !this.props.shouldRecordAudioAndVideo }
                        id = 'recording-switch-juncto'
                        onChange = { this._onRecordingServiceSwitchChange } />
                ) : null;

        const label = isVpaas ? t('recording.serviceDescriptionCloud') : t('recording.serviceDescription');
        const junctoContentRecordingIconContainer
            = integrationsEnabled || _localRecordingAvailable
                ? 'juncto-content-recording-icon-container-with-switch'
                : 'juncto-content-recording-icon-container-without-switch';
        const contentRecordingClass = isVpaas
            ? 'cloud-content-recording-icon-container'
            : junctoContentRecordingIconContainer;
        const junctoRecordingHeaderClass = !isVpaas && 'juncto-recording-header';

        return (
            <Container
                className = { `recording-header ${junctoRecordingHeaderClass}` }
                ...
                <label
                    className = 'recording-title'
                    htmlFor = 'recording-switch-juncto'>
                    { label }
                </label>
                { switchContent }
            </Container>
    ...
    _renderUploadToTheCloudInfo() {
        ...
        if (!(isVpaas && selectedRecordingService === RECORDING_TYPES.JUNCTO_REC_SERVICE) || _hideStorageWarning) {
            return null;
        }
