...
const LocalRecordingManager: ILocalRecordingManager = {
    ...
    /**
     * Returns a filename based ono the Juncto room name in the URL and timestamp.
     *
     * @returns {string}
     * */
    getFilename() {
    ...
    async startLocalRecording(store, onlySelf) {
        ...
        if (onlySelf) {
            const audioTrack: MediaStreamTrack | undefined = getLocalTrack(tracks, MEDIA_TYPE.AUDIO)?.junctoTrack?.track;
            let videoTrack: MediaStreamTrack | undefined = getLocalTrack(tracks, MEDIA_TYPE.VIDEO)?.junctoTrack?.track;
            ...
            if (supportsCaptureHandle) {
                ...
                const matchesHandle = (supportsCaptureHandle // @ts-ignore
                    && gdmVideoTrack.getCaptureHandle()?.handle === `JunctoMeet-${tabId}`);
            ...
            const localAudioTrack = getLocalTrack(tracks, MEDIA_TYPE.AUDIO)?.junctoTrack?.track;
