...
class RecordingExpandedLabel extends ExpandedLabel<IProps> {
    ...
    _getLabel() {
        ...
        switch (mode) {
        case JunctoRecordingConstants.mode.STREAM:
            prefix = 'liveStreaming';
            break;
        case JunctoRecordingConstants.mode.FILE:
            prefix = 'recording';
            break;
        }

        switch (_status) {
        case JunctoRecordingConstants.status.OFF:
            postfix = 'expandedOff';
            break;
        case JunctoRecordingConstants.status.PENDING:
            postfix = 'expandedPending';
            break;
        case JunctoRecordingConstants.status.ON:
            postfix = 'expandedOn';
            break;
        }
        ...
        if (this.props._isTranscribing) {
            if (_status === JunctoRecordingConstants.status.ON) {
