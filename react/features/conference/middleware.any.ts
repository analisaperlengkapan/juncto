...
function _checkIframe(state: IReduxState, dispatch: IStore['dispatch']) {
    ...
    if (isEmbedded() && state['features/base/config'].disableIframeAPI && !isVpaasMeeting(state) && !allowIframe) {
        ...
        const mapping: Record<string, string> = {
            'Juncto.vc': 'https://jaas.Juncto.vc',
            'meet.juncto.net': 'https://juncto.org/jaas'
        };

        const junctoserviceDomain = mapping[hostname];

        if (junctoserviceDomain) {
        ...
        if (browser.isReactNative()) {
            ...
            setTimeout(() => {
                ...
                dispatch(redirectToStaticPage('static/close3.html', `#juncto_meet_external_api_id=${API_ID}`));
            }, IFRAME_DISABLED_TIMEOUT_MINUTES * 60 * 1000);
        }
    }
}
