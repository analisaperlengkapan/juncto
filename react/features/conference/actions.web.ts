...
import { getJunctoMeetGlobalNSConnectionTimes } from '../base/util/helpers';
...
export function setupInitialDevices(recordTimeMetrics = false) {
    return async (dispatch: IStore['dispatch']) => {
        if (recordTimeMetrics) {
            getJunctoMeetGlobalNSConnectionTimes()['setupInitialDevices.start'] = window.performance.now();
        }

        await dispatch(getAvailableDevices());

        if (recordTimeMetrics) {
            getJunctoMeetGlobalNSConnectionTimes()['setupInitialDevices.getAD.finished'] = window.performance.now();
        }
        ...
        if (recordTimeMetrics) {
            getJunctoMeetGlobalNSConnectionTimes()['setupInitialDevices.end'] = now;
        }
 ...
 * prejoin is enabled.
 * @returns {Promise<JunctoConnection>}
 */
