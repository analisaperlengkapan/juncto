...
MiddlewareRegistry.register(store => next => action => {
    ...
    case CONFERENCE_FAILED: {
        ...
        // JunctoConferenceErrors.AUTHENTICATION_REQUIRED.
        let recoverable;
        // eslint-disable-next-line @typescript-eslint/no-unused-vars
        const [ _lobbyJid, lobbyWaitingForHost ] = error.params;

        if (error.name === JunctoConferenceErrors.AUTHENTICATION_REQUIRED
            || (error.name === JunctoConferenceErrors.MEMBERS_ONLY_ERROR && lobbyWaitingForHost)) {
        ...
        if (error.name === JunctoConferenceErrors.MEMBERS_ONLY_ERROR && lobbyWaitingForHost) {
            if (recoverable) {
                store.dispatch(enableModeratorLogin());
            } else {
                store.dispatch(disableModeratorLogin());
            }
        } else if (error.name === JunctoConferenceErrors.AUTHENTICATION_REQUIRED) {
            if (recoverable) {
    ...
    case CONNECTION_FAILED: {
        ...
        if (error
                && error.name === JunctoConnectionErrors.PASSWORD_REQUIRED
