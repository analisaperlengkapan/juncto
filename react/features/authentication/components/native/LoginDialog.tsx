...
import { JunctoConnectionErrors } from '../../../base/lib-juncto';
...
interface IProps {
    ...
    /**
     * {@link JunctoConference} That needs authentication - will hold a valid
     * value in XMPP login + guest access mode.
     */
    _conference?: IJunctoConference;
 ...
 * 'JunctoConnectionErrors.PASSWORD_REQUIRED' error. After username and password
 ...
 * yet, Juncofo will not allow to start new conference. This will trigger
 * 'CONFERENCE_FAILED' action with JunctoConferenceErrors.AUTHENTICATION_REQUIRED
 * error and 'authRequired' value of 'features/base/conference' will hold
 * the {@link JunctoConference} instance. If user decides to authenticate, a
 * new/separate XMPP connection is established and authentication is performed.
 * In case it succeeds, Juncofo will assign new session ID which then can be used
 * from the anonymous domain connection to create and join the room. This part
 * is done by {@link JunctoConference#authenticateAndUpgradeRole} in
 * lib-juncto.
 *
 * See {@link https://github.com/juncto/juncofo#secure-domain} for a description
...
class LoginDialog extends Component<IProps, IState> {
    ...
    _renderMessage() {
        ...
        if (progress && progress < 1) {
            ...
            if (name === JunctoConnectionErrors.PASSWORD_REQUIRED) {
