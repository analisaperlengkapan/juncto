...
StateListenerRegistry.register(
    ...
    (conference, { dispatch }, previousConference) => {
        if (conference && !previousConference) {
            conference.on(JunctoConferenceEvents.BREAKOUT_ROOMS_MOVE_TO_ROOM, (roomId: string) => {
                logger.debug(`Moving to room: ${roomId}`);
                dispatch(moveToRoom(roomId));
            });

            conference.on(JunctoConferenceEvents.BREAKOUT_ROOMS_UPDATED, ({ rooms, roomCounter }: {
