...
import { getLocalJunctoDesktopTrack } from '../base/tracks/functions';
...
export function toggleScreenshotCaptureSummary(enabled: boolean) {
    return async function(dispatch: IStore['dispatch'], getState: IStore['getState']) {
        ...
        if (state['features/screenshot-capture'].capturesEnabled !== enabled) {
            ...
            if (enabled) {
                try {
                    const junctoTrack = getLocalJunctoDesktopTrack(state);

                    await screenshotSummary.start(junctoTrack);
