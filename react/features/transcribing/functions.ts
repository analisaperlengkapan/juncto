...
import JUNCTO_TO_BCP47_MAP from './juncto-bcp47-map.json';
...
export function determineTranscriptionLanguage(config: IConfig) {
    ...
    // config BCP47 value.
    // Juncto language detections uses custom language tags, but the transcriber expects BCP-47 compliant tags,
    // we use a mapping file to convert them.
    const bcp47Locale = transcription?.useAppLanguage ?? true
        ? JUNCTO_TO_BCP47_MAP[i18next.language as keyof typeof JUNCTO_TO_BCP47_MAP]
