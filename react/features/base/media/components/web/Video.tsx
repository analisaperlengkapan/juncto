...
interface IProps {
    ...
    /**
     * The JunctoLocalTrack to display.
     */
...
class Video extends Component<IProps> {
                ...
                .catch(error => {
                    ...
                    if (this._mounted) {
                        logger.error(`Error while trying to play video with id ${
                            this.props.id} and video track ${this.props.videoTrack?.junctoTrack}: ${error}`);
                    }
                });
            }
        });
    ...
    override shouldComponentUpdate(nextProps: IProps) {
        const currentJunctoTrack = this.props.videoTrack?.junctoTrack;
        const nextJunctoTrack = nextProps.videoTrack?.junctoTrack;
    ...
    _attachTrack(videoTrack?: Partial<ITrack>) {
        ...
        return videoTrack.junctoTrack.attach(this._videoElement)
            .catch((error: Error) => {
                logger.error(
                    `Attaching the remote track ${videoTrack.junctoTrack} to video with id ${id} has failed with `,
                    error);
            });
    }
     ...
     * in redux representation of juncto video track to stop the track from
     * rendering.
     *
     * @param {Object} videoTrack -  The redux representation of the
     * {@code JunctoLocalTrack}.
    ...
    _detachTrack(videoTrack?: Partial<ITrack>) {
        if (this._videoElement && videoTrack?.junctoTrack) {
            videoTrack.junctoTrack.detach(this._videoElement);
        }
    }
     ...
     * referenced later for attaching and detaching a JunctoLocalTrack.
     *
