...
class AudioTrack extends Component<IProps> {
    ...
    override shouldComponentUpdate(nextProps: IProps) {
        const currentJunctoTrack = this.props.audioTrack?.junctoTrack;
        const nextJunctoTrack = nextProps.audioTrack?.junctoTrack;

        if (currentJunctoTrack !== nextJunctoTrack) {
            this._detachTrack(this.props.audioTrack);
     ...
     *
     * @param {Object} track - The redux representation of the {@code JunctoLocalTrack}.
     * @private
     * @returns {void}
     */
    _attachTrack(track?: ITrack) {
        ...
        if (!this._ref?.current) {
            logger.warn(`Attempting to attach track ${track?.junctoTrack} on AudioTrack ${id} without reference!`);

            return;
        }

        track.junctoTrack.attach(this._ref.current)
            .catch((error: Error) => {
                logger.error(
                    `Attaching the remote track ${track.junctoTrack} to video with id ${id} has failed with `,
                    error);
            })
            .finally(() => {
                this._play();
     ...
     * in redux representation of juncto audio track.
     *
     * @param {Object} track -  The redux representation of the {@code JunctoLocalTrack}.
     * @private
     * @returns {void}
    ...
    _detachTrack(track?: ITrack) {
        if (this._ref?.current && track?.junctoTrack) {
            clearTimeout(this._playTimeout);
            this._playTimeout = undefined;
            track.junctoTrack.detach(this._ref.current);
        }
    }
    ...
    _errorHandler(error: Error) {
        logger.error(`Error ${error?.message} called on audio track ${this.props.audioTrack?.junctoTrack}. `
