...
import { JunctoTrackErrors, JunctoTrackEvents } from '../lib-juncto';
 ...
 *
 * @param {JunctoLocalTrack} newTrack - The local track to be added to the conference.
 * @returns {Function}
 */
 ...
 *
 * @param {Object} [options] - For info @see JunctoMeetJS.createLocalTracks.
 * @returns {Function}
 */
            ...
            const gumProcess: any
                ...
                .then( // @ts-ignore
                    (localTracks: any[]) => {
                        ...
                        // should not get more than one JunctoTrack.
...
/**
 * Calls JunctoLocalTrack#dispose() on the given track or on all local tracks (if none are passed) ignoring errors if
 * track is already disposed. After that signals tracks to be removed.
 *
 * @param {JunctoLocalTrack|null} [track] - The local track that needs to be destroyed.
 * @returns {Function}
 */
...
/**
 * Signals that the passed JunctoLocalTrack has triggered a no data from source event.
 *
 * @param {JunctoLocalTrack} track - The track.
 * @returns {{
 ...
 *
 * @param {JunctoLocalTrack} junctoTrack - The track.
 * @returns {Function}
 */
export function showNoDataFromSourceVideoError(junctoTrack: any) {
    return (dispatch: IStore['dispatch'], getState: IStore['getState']) => {
        let notificationInfo;

        const track = getTrackByJunctoTrack(getState()['features/base/tracks'], junctoTrack);
        ...
        dispatch(trackNoDataFromSourceNotificationInfoChanged(junctoTrack, notificationInfo));
    };
}
 ...
 *
 * @param {JunctoLocalTrack|null} oldTrack - The track to dispose.
 * @param {JunctoLocalTrack|null} newTrack - The track to use instead.
 * @param {JunctoConference} [conference] - The conference from which to remove
 * and add the tracks. If one is not provided, the conference in the redux store
 * will be used.
 * @returns {Function}
 */
export function replaceLocalTrack(oldTrack: any, newTrack: any, conference?: IJunctoConference) {
    return async (dispatch: IStore['dispatch'], getState: IStore['getState']) => {
        conference
...
function replaceStoredTracks(oldTrack: any, newTrack: any) {
    return async (dispatch: IStore['dispatch']) => {
        ...
        // after means the JunctoLocalTrack.conference is already
 ...
 * conference.
 *
 * @param {(JunctoLocalTrack|JunctoRemoteTrack)} track - JunctoTrack instance.
 * @returns {Function}
 */
export function trackAdded(track: any) {
    return (dispatch: IStore['dispatch'], getState: IStore['getState']) => {
        track.on(
            JunctoTrackEvents.TRACK_MUTE_CHANGED,
            () => dispatch(trackMutedChanged(track)));
        track.on(
            JunctoTrackEvents.TRACK_VIDEOTYPE_CHANGED,
        ...
        if (local) {
            ...
            track.on(JunctoTrackEvents.NO_DATA_FROM_SOURCE, () => dispatch(noDataFromSource({ junctoTrack: track })));
            if (!isReceivingData) {
            ...
            track.on(JunctoTrackEvents.LOCAL_TRACK_STOPPED,
                () => {
 ...
 *
 * @param {JunctoLocalTrack} track - JunctoLocalTrack instance.
 ...
 *
 * @param {JunctoLocalTrack} track - JunctoTrack instance.
...
export function trackRemoved(track: any): {
    ...
    track.removeAllListeners(JunctoTrackEvents.TRACK_MUTE_CHANGED);
    track.removeAllListeners(JunctoTrackEvents.TRACK_VIDEOTYPE_CHANGED);
    track.removeAllListeners(JunctoTrackEvents.NO_DATA_FROM_SOURCE);
 ...
 *
 * @param {(JunctoRemoteTrack)} track - JunctoTrack instance.
 ...
 *
 * @param {(JunctoLocalTrack|JunctoRemoteTrack)[]} tracks - List of tracks.
 * @private
 * @returns {Function}
 */
 ...
 * @param {(JunctoLocalTrack|JunctoRemoteTrack)[]} tracks - List of tracks.
 * @private
 * @returns {Promise} - A Promise resolved once {@link JunctoTrack.dispose()} is
        ...
        tracks.map(t =>
            t.dispose()
                .catch((err: Error) => {
                    ...
                    if (err.name !== JunctoTrackErrors.TRACK_IS_DISPOSED) {
                        throw err;
                    }
                })));
}
...
/**
 * Returns true if the provided {@code JunctoTrack} should be rendered as a
 * mirror.
 *
 ...
 *
 * TODO Similar functionality is part of lib-juncto. This function should be
 * removed after https://github.com/juncto/lib-juncto/pull/187 is merged.
 *
 * @param {(JunctoLocalTrack|JunctoRemoteTrack)} track - JunctoTrack instance.
 * @private
 * @returns {boolean}
...
function _shouldMirror(track: any): boolean {
    return (
        track?.isLocal()
            ...
            // XXX The type of the return value of JunctoLocalTrack's
            // getCameraFacingMode happens to be named CAMERA_FACING_MODE as
            // well, it's defined by lib-juncto. Note though that the type
            // of the value on the right side of the equality check is defined
            // by juncto. The type definitions are surely compatible today
...
export function destroyLocalDesktopTrackIfExists() {
    return (dispatch: IStore['dispatch'], getState: IStore['getState']) => {
        ...
        if (isDesktopTrack) {
            dispatch(_disposeAndRemoveTracks([ videoTrack.junctoTrack ]));
        }
    };
}
