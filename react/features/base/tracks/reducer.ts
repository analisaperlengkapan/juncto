...
function track(state: ITrack, action: AnyAction) {
    ...
    case TRACK_UPDATED: {
        ...
        if (state.junctoTrack === t.junctoTrack) {
    ...
    case TRACK_NO_DATA_FROM_SOURCE: {
        ...
        if (state.junctoTrack === t.junctoTrack) {
            const isReceivingData = t.junctoTrack.isReceivingData();
...
ReducerRegistry.register<ITracksState>('features/base/tracks', (state = [], action): ITracksState => {
    ...
    case TRACK_REMOVED:
        return state.filter((t: ITrack) => t.junctoTrack !== action.track.junctoTrack);
