...
async function _startScreenSharing(dispatch: IStore['dispatch'], state: IReduxState) {
    try {
        const tracks: any[] = await JunctoMeetJS.createLocalTracks({ devices: [ 'desktop' ] });
        const track = tracks[0];
        const currentLocalDesktopTrack = getLocalDesktopTrack(getTrackState(state));
        const currentJunctoTrack = currentLocalDesktopTrack?.junctoTrack;
        ...
        if (currentJunctoTrack) {
            dispatch(replaceLocalTrack(currentJunctoTrack, track));
        } else {
