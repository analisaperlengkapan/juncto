...
import { JunctoTrackErrors, browser } from '../lib-juncto';
 ...
 * {@code junctoTrack} property is {@code undefined}. By default a pending local track is not returned.
 * @returns {(Track|undefined)}
 */
...
/**
 * Returns the stored local desktop junctoLocalTrack.
 *
 * @param {IReduxState} state - The redux state.
 * @returns {JunctoLocalTrack|undefined}
 */
export function getLocalJunctoDesktopTrack(state: IReduxState) {
    const track = getLocalDesktopTrack(getTrackState(state));

    return track?.junctoTrack;
}
 ...
 * {@code junctoTrack} property is {@code undefined}. By default a pending local
...
export function getLocalTracks(tracks: ITrack[], includePending = false) {
    // XXX A local track is considered ready only once it has its `junctoTrack`
    ...
    return tracks.filter(t => t.local && (t.junctoTrack || includePending));
}
 ...
 * @returns {Object}
 */
export function getLocalJunctoVideoTrack(state: IReduxState) {
 ...
 * @returns {Object}
 */
export function getLocalJunctoAudioTrack(state: IReduxState) {
...
/**
 * Returns audio settings from the local Juncto audio track.
 *
 * @param {IReduxState} state - The Redux state.
 * @returns {IAudioSettings} The extracted audio settings.
 */
export function getLocalJunctoAudioTrackSettings(state: IReduxState) {
    const junctoTrack = getLocalJunctoAudioTrack(state);
    ...
    const hasAudioMixerEffect = Boolean(typeof junctoTrack._streamEffect?.setMuted === 'function' && junctoTrack._streamEffect?._originalTrack);

    const track = hasAudioMixerEffect ? junctoTrack._streamEffect._originalTrack : junctoTrack.getTrack();
...
export function getTrackByMediaTypeAndParticipant(
    ...
    return tracks.find(
        t => Boolean(t.junctoTrack) && t.participantId === participantId && t.mediaType === mediaType
    );
}
...
export function getTrackSourceNameByMediaTypeAndParticipant(
    ...
    return track?.junctoTrack?.getSourceName();
}

/**
 * Returns the track if any which corresponds to a specific instance
 * of JunctoLocalTrack or JunctoRemoteTrack.
 *
 * @param {ITrack[]} tracks - List of all tracks.
 * @param {(JunctoLocalTrack|JunctoRemoteTrack)} junctoTrack - JunctoTrack instance.
 * @returns {(Track|undefined)}
 */
export function getTrackByJunctoTrack(tracks: ITrack[], junctoTrack: any) {
    return tracks.find(t => t.junctoTrack === junctoTrack);
}
...
/**
 * Mutes or unmutes a specific {@code JunctoLocalTrack}. If the muted state of the specified {@code track} is already in
 * accord with the specified {@code muted} value, then does nothing.
 *
 * @param {JunctoLocalTrack} track - The {@code JunctoLocalTrack} to mute or unmute.
...
export function setTrackMuted(track: any, muted: boolean, state: IReduxState | IMediaState,
    ...
    .catch((error: Error) => {
        ...
        if (error.name !== JunctoTrackErrors.TRACK_IS_DISPOSED) {
...
export function logTracksForParticipant(tracksState: ITrack[], participantId: string, reason?: string) {
    ...
    const trackStateStrings = tracks.map(t => `{type: ${t.mediaType}, videoType: ${t.videoType}, muted: ${
        t.muted}, isReceivingData: ${t.isReceivingData}, junctoTrack: ${t.junctoTrack?.toString()}}`);
