...
import JunctoMeetJS, { JunctoTrackErrors, browser } from '../lib-juncto';
...
import { getJunctoMeetGlobalNSConnectionTimes } from '../util/helpers';

import { getCameraFacingMode, getLocalJunctoAudioTrack, getLocalJunctoAudioTrackSettings } from './functions.any';
 ...
 * {@code undefined} to use app's settings.
 * @param {number|undefined} [options.timeout] - A timeout for JunctoMeetJS.createLocalTracks used to create the tracks.
...
export function createLocalTracksF(options: ITrackOptions = {}, store?: IStore, recordTimeMetrics = false) {
    ...
    if (isAdvancedAudioSettingsEnabled(state) && typeof APP !== 'undefined') {
        constraints.audio = state['features/settings'].audioSettings ?? getLocalJunctoAudioTrackSettings(state);
    }
    ...
    return (
        loadEffects(store).then((effectsArray: Object[]) => {
            if (recordTimeMetrics) {
                getJunctoMeetGlobalNSConnectionTimes()['trackEffects.loaded'] = window.performance.now();
            }

            // Filter any undefined values returned by Promise.resolve().
            const effects = effectsArray.filter(effect => Boolean(effect));

            return JunctoMeetJS.createLocalTracks(
                {
                    cameraDeviceId,
                    constraints,
                    desktopSharingFrameRate,
...
export function createPrejoinTracks() {
    ...
    if (requestedAudio || requestedVideo) {
        ...
        .catch(async (err: Error) => {
            if (err.name === JunctoTrackErrors.TIMEOUT && !browser.isElectron()) {
...
/**
 * Applies audio constraints to the local Juncto audio track.
 *
 * @param {Function|Object} stateful - The redux store or {@code getState} function.
...
export async function applyAudioConstraints(stateful: IStateful, settings: IAudioSettings) {
    const state = toState(stateful);
    const track = getLocalJunctoAudioTrack(state);
