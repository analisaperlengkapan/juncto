...
MiddlewareRegistry.register(store => next => action => {
    ...
    case SET_CAMERA_FACING_MODE: {
        ...
        if (localTrack
                && (junctoTrack = localTrack.junctoTrack)
                && junctoTrack.getCameraFacingMode()
    ...
    case TOGGLE_CAMERA_FACING_MODE: {
        ...
        if (localTrack && (junctoTrack = localTrack.junctoTrack)) {
            ...
            junctoTrack._switchCamera();

            // Don't mirror the video of the back/environment-facing camera.
            const mirror
                = junctoTrack.getCameraFacingMode() === CAMERA_FACING_MODE.USER;
        ...
        if (prevConference && !conference && !authRequired && !error) {
            ...
            batch(() => {
                ...
                for (const track of remoteTracks) {
                    dispatch(trackRemoved(track.junctoTrack));
                }
