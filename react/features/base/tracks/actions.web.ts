...
import { JunctoTrackErrors, JunctoTrackEvents, browser } from '../lib-juncto';
import { createLocalTrack } from '../lib-juncto/functions.any';
...
import { applyAudioConstraints, getLocalJunctoAudioTrackSettings } from './functions.web';
 ...
 *
 * @private
 * @param {JunctoLocalTrack} desktopAudioTrack - The audio track to be added to the conference.
...
async function _toggleScreenSharing(
    ...
    if (enable) {
        ...
        if (audioOnly) {
            ...
            if (localScreenshare) {
                await dispatch(replaceLocalTrack(localScreenshare.junctoTrack, desktopVideoTrack, conference));
            } else {
...
export function setCameraFacingMode(facingMode: string | undefined) {
    return async (dispatch: IStore['dispatch'], getState: IStore['getState']) => {
        ...
        const localVideoTrack = getLocalVideoTrack(tracks)?.junctoTrack;
 ...
 *
 * @param {JunctoLocalTrack} tracks - The tracks that have been created.
...
export function createInitialAVTracks(options: ICreateInitialTracksOptions, recordTimeMetrics = false) {
    return (dispatch: IStore['dispatch'], _getState: IStore['getState']) => {
        ...
        .catch(async error => {
            ...
            if (error.name === JunctoTrackErrors.TIMEOUT && !browser.isElectron()) {
...
export function handleScreenSharingError(
    ...
    return (dispatch: IStore['dispatch']) => {
        ...
        if (error.name === JunctoTrackErrors.PERMISSION_DENIED) {
            descriptionKey = 'dialog.screenSharingPermissionDeniedError';
            titleKey = 'dialog.screenSharingFailedTitle';
        } else if (error.name === JunctoTrackErrors.CONSTRAINT_FAILED) {
            descriptionKey = 'dialog.cameraConstraintFailedError';
            titleKey = 'deviceError.cameraError';
        } else if (error.name === JunctoTrackErrors.SCREENSHARING_GENERIC_ERROR) {
            ...
            // error.name === JunctoTrackErrors.SCREENSHARING_USER_CANCELED
            return;
        }
...
export function toggleUpdateAudioSettings(settings: IAudioSettings) {
    return async (dispatch: IStore['dispatch'], getState: IStore['getState']) => {
        ...
        const updatedSettings = getLocalJunctoAudioTrackSettings(state) as IAudioSettings;
