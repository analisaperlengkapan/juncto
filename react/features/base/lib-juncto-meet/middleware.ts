 ...
 * (which signalizes that we finally left the app) and disposes lib-juncto.
 * Also captures SET_CONFIG action and disposes previous instance (if any) of
 * lib-juncto, and initializes a new one with new config.
 *
...
/**
 * Notifies the feature base/lib-juncto that the action SET_CONFIG is being
...
function _setConfig({ dispatch, getState }: IStore, next: Function, action: AnyAction) {
    const { initialized } = getState()['features/base/lib-juncto'];

    // XXX Since the config is changing, the library lib-juncto must be
    // initialized again with the new config. Consequently, it may need to be
    // disposed of first.
    // TODO Currently, disposeLib actually does not dispose of lib-juncto
    // because lib-juncto does not implement such functionality.
    if (initialized) {
        dispatch(disposeLib());
    }
