...
import { LIB_WILL_INIT } from '../lib-juncto/actionTypes';
...
import JunctoMeetInMemoryLogStorage from './JunctoMeetInMemoryLogStorage';
import JunctoMeetLogStorage from './JunctoMeetLogStorage';
...
function _appWillMount({ getState }: IStore, next: Function, action: AnyAction) {
    ...
    // app we, JunctoMeetJS.init is to not be used for the React app.
    // Consequently, LIB_WILL_INIT will not be dispatched. In the meantime, do
    // the following:
    typeof APP === 'undefined' || _setLogLevels(JunctoMeetJS, config);
...
function _conferenceJoined({ getState }: IStore, next: Function, action: AnyAction) {
    ...
    // Wait until the joined event is processed, so that the JunctoMeetLogStorage
    // will be ready.
    const result = next(action);
    ...
    if (logCollector && conference === getCurrentConference(getState())) {
        ...
        // statistics module is stopped). That includes the case when Juncofo
        ...
        // again.
        conference.on(
            JunctoConferenceEvents.BEFORE_STATISTICS_DISPOSED,
            () => logCollector.flush()
        );
    }
...
function _initLogging({ dispatch, getState }: IStore,
    ...
    // cached, before the JunctoMeetLogStorage gets ready (RTCStats trace is
    // available).
    if (!logCollector && !loggingConfig.disableLogCollector) {
        ...
        const _logCollector = new Logger.LogCollector(new JunctoMeetLogStorage(getState), {
        ...
        if (apiLogLevels && Array.isArray(apiLogLevels) && typeof APP === 'object') {
            ...
            JunctoMeetJS.addGlobalLogTransport(transport);
        }
        ...
        // The JunctoMeetInMemoryLogStorage can not be accessed on mobile through
        // the 'executeScript' method like it's done in torture tests for WEB.
        if (isTestingEnabled && typeof APP === 'object') {
            APP.debugLogs = new JunctoMeetInMemoryLogStorage();
...
function _libWillInit({ getState }: IStore, next: Function, action: AnyAction) {
    ...
    if (typeof APP === 'undefined') {
        _setLogLevels(JunctoMeetJS, getState()['features/base/logging'].config);
    }
...
function _setLoggingConfig({ dispatch, getState }: IStore,
    ...
    _setLogLevels(JunctoMeetJS, newValue);
...
/**
 * Sets the log levels of {@link Logger} or {@link JunctoMeetJS} in accord with
 ...
 * set on {@code Logger} or {@code JunctoMeetJS}.
 * @private
 * @returns {void}
 */
function _setLogLevels(logger: any, config: any) {
    // XXX The loggers of the library lib-juncto and the application
    // juncto are separate, so the log levels have to be set in both.
