...
/**
 * Implements log storage interface from the @juncto/logger lib, as it stands
 * now it only sends logs to the rtcstats server in case it is enabled.
 */
export default class JunctoMeetLogStorage {
    getState: IStore['getState'];

    /**
     * Creates new <tt>JunctoMeetLogStorage</tt>.
     *
    ...
    /**
     * The JunctoMeetLogStorage is ready we can use the rtcstats trace to send logs
     ...
     */
    isReady() {
        return JunctoMeetJS.rtcstats.isTraceAvailable();
    }
    ...
    canStoreLogsRtcstats() {
        ...
        // Be mindful that there exists another LogStorage instance withing lib-juncto,
