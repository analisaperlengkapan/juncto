...
interface IProps extends WithTranslation {
    ...
    /**
     * If the Juncto watermark should be displayed or not.
     */
    _showJunctoWatermark: boolean;

    /**
     * The default value for the Juncto logo URL.
     */
    defaultJunctoLogoURL?: string;
...
type State = {
    ...
    /**
     * Whether or not the show the "powered by Juncto.org" link.
     */
...
class Watermarks extends Component<IProps, State> {
    ...
    /**
     * Renders a Juncto watermark if it is enabled.
     *
     * @private
     * @returns {ReactElement|null}
     */
    _renderJunctoWatermark() {
        const {
            _logoLink,
            _logoUrl,
            _showJunctoWatermark
        } = this.props;
        ...
        if (_showJunctoWatermark) {
            ...
            if (_logoLink) {
                reactElement = (
                    <a
                        aria-label = { t('junctoHome', { logo: interfaceConfig.APP_NAME }) }
                        className = { className }
                        href = { _logoLink }
                        target = '_new'>
                        { reactElement }
    ...
    _renderPoweredBy() {
        if (this.state.showPoweredBy) {
            ...
            return (
                <a
                    className = 'poweredby'
                    href = 'http://juncto.org'
                    target = '_new'>
                    <span>{ t('poweredby') } juncto.org</span>
                </a>
            );
        }
...
function _mapStateToProps(state: IReduxState, ownProps: any) {
    ...
    const {
        JUNCTO_WATERMARK_LINK,
        SHOW_JUNCTO_WATERMARK
    } = interfaceConfig;
    let _showJunctoWatermark = (
    ...
    if (useDynamicBrandingData) {
        if (isVpaasMeeting(state)) {
            ...
            _showJunctoWatermark = !customizationFailed && Boolean(logoImageUrl);
        } else if (defaultBranding) {
            _logoUrl = defaultLogoUrl;
            _logoLink = JUNCTO_WATERMARK_LINK;
        }
    } else {
        // When there is no custom branding data use defaults
        _logoUrl = ownProps.defaultJunctoLogoURL || defaultLogoUrl;
        _logoLink = JUNCTO_WATERMARK_LINK;
    }
