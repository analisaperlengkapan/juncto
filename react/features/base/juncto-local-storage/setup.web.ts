...
// @ts-ignore
import { safeJsonParse } from '@juncto/js-utils/json';
import { pick } from 'lodash-es';

import { browser } from '../lib-juncto';
...
function onFakeLocalStorageChanged() {
    APP.API.notifyLocalStorageChanged(junctoLocalStorage.serialize([ 'junctoLocalStorage' ]));
}

/**
 * Checks if the local storage of the host page needs to be used instead juncto's local storage.
 *
 * @param {Object} urlParams - Object with parsed URL params.
 * @returns {boolean} - True if the local storage of the host page needs to be used instead juncto's local storage
 * and false otherwise.
 */
function shouldUseHostPageLocalStorage(urlParams: { 'config.useHostPageLocalStorage'?: boolean; }) {
    ...
    if (junctoLocalStorage.isLocalStorageDisabled()) { // We have detected that ou own local storage is not working.
        return true;
    }
...
/**
 * Performs initial setup of the junctoLocalStorage.
 *
 * @returns {void}
 */
function setupJunctoLocalStorage() {
    // @ts-ignore
    ...
    if (shouldUseHostPageLocalStorage(urlParams)) {
        try {
            ...
            // local storage from junctoLocalStorage and we will loose the data.
            junctoLocalStorage.setLocalStorageDisabled(true);
            ...
            if (typeof localStorageContent === 'object') {
                ...
                Object.keys(localStorageContent).forEach(key => {
                    junctoLocalStorage.setItem(key, localStorageContent[key]);
                });
            }
        } catch (error) {
        ...
        junctoLocalStorage.on('changed', onFakeLocalStorageChanged);
    }
}

setupJunctoLocalStorage();
