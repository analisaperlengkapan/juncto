...
function _initSettings(featureState: ISettingsState) {
    ...
    // FIXME: junbri uses old settings.js local storage values to set its display
    // name and email. Provide another way for junbri to set these values, update
    // junbri, and remove the old settings.js values.
    const savedDisplayName = junctoLocalStorage.getItem('displayname');
    const savedEmail = junctoLocalStorage.getItem('email');
