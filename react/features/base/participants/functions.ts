// @ts-expect-error
import { getGravatarURL } from '@juncto/js-utils/avatar';
...
import { FakeParticipant, IJunctoParticipant, IParticipant, ISourceInfo } from './types';
...
const AVATAR_CHECKER_FUNCTIONS = [
    (participant: IParticipant) => {
        return participant?.isJungasi ? JIGASI_PARTICIPANT_ICON : null;
    },
    (participant: IParticipant) => {
...
export function getVirtualScreenshareParticipantByOwnerId(stateful: IStateful, id: string) {
    ...
    return getParticipantById(stateful, track?.junctoTrack.getSourceName());
}
 ...
 * JUNCTOBRIDGE will add the source to the video sources map and signal it to the local endpoint. Only then, a remote track is
...
export function getRemoteScreensharesBasedOnPresence(stateful: IStateful): string[] {
    ...
    return conference?.getParticipants()?.reduce((screenshares: string[], participant: IJunctoParticipant) => {
