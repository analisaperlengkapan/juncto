...
import { FakeParticipant, IJunctoParticipant, IParticipant } from './types';
 ...
 * @param {JunctoConference} conference - The {@code JunctoConference} associated
 * with the participant identified by the specified {@code id}. Only the local
 * participant is allowed to not specify an associated {@code JunctoConference}
 * instance.
 * @returns {{
...
export function dominantSpeakerChanged(
        dominantSpeaker: string, previousSpeakers: string[], silence: boolean, conference: IJunctoConference) {
    return {
        type: DOMINANT_SPEAKER_CHANGED,
        participant: {
            conference,
            ...
            return (
                dispatch(
                    participantLeft(
                        ...
                        // without stating the JunctoConference instance because
...
export function participantJoined(participant: IParticipant) {
    ...
    if (!conference) {
        throw Error(
            'A remote participant must be associated with a JunctoConference!');
    }
    ...
    return (dispatch: IStore['dispatch'], getState: IStore['getState']) => {
        ...
        // JunctoConference may have moved into leaving but may still manage to
 ...
 *
 * @param {IJunctoParticipant} junctoParticipant - The IJunctoParticipant instance.
 * @returns {{
 *      type: PARTICIPANT_SOURCES_UPDATED,
 *      participant: IParticipant
 * }}
 */
export function participantSourcesUpdated(junctoParticipant: IJunctoParticipant) {
    return (dispatch: IStore['dispatch'], getState: IStore['getState']) => {
        const id = junctoParticipant.getId();
        const participant = getParticipantById(getState(), id);

        if (participant?.local) {
            return;
        }
        const sources = junctoParticipant.getSources();
 ...
 *
 * @param {JunctoParticipant} junctoParticipant - The ID of the participant.
 * @returns {{
*     type: PARTICIPANT_UPDATED,
*     participant: IParticipant
* }}
*/
export function updateRemoteParticipantFeatures(junctoParticipant: any) {
    return (dispatch: IStore['dispatch'], getState: IStore['getState']) => {
        if (!junctoParticipant) {
            return;
        }

        const id = junctoParticipant.getId();

        junctoParticipant.getFeatures()
 ...
 * }}
 */
export function participantLeft(id: string, conference?: IJunctoConference, participantLeftProps: {
 ...
 *
 * @param {JunctoParticipant} participant - Information about participant.
 * @param {JunctoLocalTrack} track - Information about the track that has been muted.
 * @returns {Promise}
 */
 ...
 *
 * @param {(string)} sourceName - The source name of the JunctoTrack instance.
 * @param {(boolean)} local - Whether it's a local or remote participant.
 * @param {JunctoConference} conference - The conference instance for which the participant is to be created.
 * @returns {Function}
 */
export function createVirtualScreenshareParticipant(sourceName: string, local: boolean, conference?: IJunctoConference) {
    return (dispatch: IStore['dispatch'], getState: IStore['getState']) => {
 ...
 *
 * @param {JunctoParticipant} kicker - Information about participant performing the kick.
 * @param {JunctoParticipant} kicked - Information about participant that was kicked.
 * @returns {Promise}
 */
