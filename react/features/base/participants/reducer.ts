...
(state = DEFAULT_STATE, action): IParticipantsState => {
    ...
    case PARTICIPANT_LEFT: {
        ...
        // specific JunctoConference instance. The local participant is uniquely
 ...
 * @param {IParticipant} action.participant - Information about participant to be
 * added/modified.
 * @param {JunctoConference} action.conference - Conference instance.
 * @private
 * @returns {IParticipant}
 */
...
function _participantJoined({ participant }: { participant: IParticipant; }) {
    ...
    if (local) {
        ...
        // JunctoConference because it is identified by the very fact that it is
