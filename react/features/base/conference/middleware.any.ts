...
import { JunctoConferenceErrors, JunctoConferenceEvents, JunctoConnectionErrors } from '../lib-juncto';
...
StateListenerRegistry.register(
    ...
    (conference, { dispatch }, previousConference): void => {
        if (conference && !previousConference) {
            conference.on(JunctoConferenceEvents.METADATA_UPDATED, (metadata: IConferenceMetadata) => {
...
function _conferenceFailed({ dispatch, getState }: IStore, next: Function, action: AnyAction) {
    ...
    switch (error.name) {
    case JunctoConferenceErrors.CONFERENCE_RESTARTED: {
    ...
    case JunctoConferenceErrors.CONNECTION_ERROR: {
    ...
    case JunctoConferenceErrors.CONFERENCE_MAX_USERS: {
    ...
    case JunctoConferenceErrors.NOT_ALLOWED_ERROR: {
        const [ type, msg ] = error.params;

        let descriptionKey, customActionNameKey, customActionHandler;
        let titleKey = 'dialog.tokenAuthFailed';

        if (type === JunctoConferenceErrors.AUTH_ERROR_TYPES.NO_MAIN_PARTICIPANTS) {
            descriptionKey = 'visitors.notification.noMainParticipantsDescription';
            titleKey = 'visitors.notification.noMainParticipantsTitle';
        } else if (type === JunctoConferenceErrors.AUTH_ERROR_TYPES.NO_VISITORS_LOBBY) {
            descriptionKey = 'visitors.notification.noVisitorLobby';
        } else if (type === JunctoConferenceErrors.AUTH_ERROR_TYPES.PROMOTION_NOT_ALLOWED) {
            descriptionKey = 'visitors.notification.notAllowedPromotion';
        } else if (type === JunctoConferenceErrors.AUTH_ERROR_TYPES.ROOM_CREATION_RESTRICTION) {
            descriptionKey = 'dialog.errorRoomCreationRestriction';
        } else if (type === JunctoConferenceErrors.AUTH_ERROR_TYPES.ROOM_UNAUTHENTICATED_ACCESS_DISABLED) {
    ...
    && conference?.leave(CONFERENCE_LEAVE_REASONS.UNRECOVERABLE_ERROR).catch((reason: Error) => {
        ...
        logger.info('JunctoConference.leave() rejected with:', reason);
    });
    ...
    if (enableForcedReload
        && (error?.name === JunctoConferenceErrors.CONFERENCE_RESTARTED
            || error?.name === JunctoConnectionErrors.SHARD_CHANGED_ERROR)) {
        dispatch(conferenceWillLeave(conference));
        dispatch(reloadNow());
    }
...
function _conferenceJoined({ dispatch, getState }: IStore, next: Function, action: AnyAction) {
    ...
    // When the user closes the window or quits the browser, lib-juncto
    ...
    if (!iAmVisitor(getState())) {
        ...
        // we need to send iq to juncofo, so it can join/create the breakout room
...
function _connectionFailed({ dispatch, getState }: IStore, next: Function, action: AnyAction) {
    ...
    if (jwt) {
        ...
        if (error.name === JunctoConnectionErrors.PASSWORD_REQUIRED) {
            dispatch(showErrorNotification({
    ...
    if (error.name === JunctoConnectionErrors.CONFERENCE_REQUEST_FAILED) {
    ...
    forEachConference(getState, conference => {
        // TODO: revisit this
        // It feels that it would make things easier if JunctoConference
        // in lib-juncto would monitor it's connection and emit
...
async function _trackAddedOrRemoved(store: IStore, next: Function, action: AnyAction) {
    ...
    if (track?.local) {
        ...
        if (conference) {
            const junctoTrack = action.track.junctoTrack;
            ...
            if (action.type === TRACK_ADDED) {
                ...
                if (!iAmVisitor(state)) {
                    ...
                    if (typeof APP !== 'undefined' && desktopAudioTrack && track.mediaType === MEDIA_TYPE.AUDIO) {
                        ...
                        await junctoTrack.setEffect(audioMixerEffect);
                        await conference.replaceTrack(null, junctoTrack);
                    } else {
                        await _addLocalTracksToConference(conference, [ junctoTrack ]);
                    }
                }
            } else {
                await _removeLocalTracksFromConference(conference, [ junctoTrack ]);
            }
        }
    }
