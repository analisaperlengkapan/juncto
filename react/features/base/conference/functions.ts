...
import { JunctoTrackErrors } from '../lib-juncto';
...
import { IJunctoConference } from './reducer';
 ...
 *
 * @param {JunctoConference} conference - Conference instance.
 * @param {JunctoLocalTrack[]} localTracks - List of local media tracks.
 * @protected
 * @returns {Promise}
 */
export function _addLocalTracksToConference(
    ...
    for (const track of localTracks) {
        // XXX The library lib-juncto may be draconian, for example, when
 ...
 * @param {JunctoMeetConference} conference - The conference for which the
 * {@code USER_JOINED} event is being processed.
 * @param {JunctoParticipant} user - The user who has just joined.
 * @returns {void}
 */
 ...
 * @param {JunctoParticipant} user - The user who has just left.
 * @returns {void}
 */
...
/**
 * Evaluates a specific predicate for each {@link JunctoConference} known to the
 ...
 * {@code JunctoConference} know to the redux state features/base/conference
 * while it returns {@code true}.
 * @returns {boolean} If the specified {@code predicate} returned {@code true}
 * for all {@code JunctoConference} instances known to the redux state
...
export function forEachConference(
    ...
    for (const v of Object.values(state)) {
        ...
        if (v && typeof v === 'object') {
            const url: URL = v[JUNCTO_CONFERENCE_URL_KEY];

            // XXX The Web version of Juncto does not utilize
            // JUNCTO_CONFERENCE_URL_KEY at the time of this writing. An
            // alternative is necessary then to recognize JunctoConference
...
export function getVisitorOptions(stateful: IStateful, vnode: string, focusJid: string, username: string) {
    ...
    if (!vnode) {
        ...
        // and the visitor will be redirected back to a vnode from juncofo
    ...
    const domain = `${vnode}.meet.juncto`;
...
/**
 * Returns the current {@code JunctoConference} which is joining or joined and is
 ...
 * @param {IStateful} stateful - The redux store, state, or
 * {@code getState} function.
 * @returns {JunctoConference|undefined}
 */
export function getCurrentConference(stateful: IStateful): IJunctoConference | undefined {
 ...
 * Will return `false` if it's a JUNCTOBRIDGE one, and `null` if there is no conference.
 *
 * @param {IStateful} stateful - The redux store, state, or
 * {@code getState} function.
...
/**
 * Handle an error thrown by the backend (i.e. {@code lib-juncto}) while
...
export function _handleParticipantError(err: Error) {
    ...
    // TODO Lib-juncto should be fixed to not throw such an exception in
    // these scenarios.
    ...
    return Promise.all(localTracks.map(track =>
        conference.removeTrack(track)
            .catch((err: Error) => {
                ...
                // JunctoTrack#dispose() call. So we should ignore this error
                // here.
 ...
 * {@code getState} function.
 * @param {JunctoConference} conference - The {@code JunctoConference} to which
