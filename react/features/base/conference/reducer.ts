...
import { JunctoConferenceErrors } from '../lib-juncto';
...
export interface IJunctoConference {
...
export interface IConferenceState {
    ...
    authEnabled?: boolean;
    authLogin?: string;
    authRequired?: IJunctoConference;
    conference?: IJunctoConference;
    conferenceTimestamp?: number;
    dataChannelOpen?: boolean;
    e2eeSupported?: boolean;
    error?: Error;
    joining?: IJunctoConference;
    leaving?: IJunctoConference;
    lobbyError?: boolean;
    lobbyWaitingForHost?: boolean;
    localSubject?: string;
    locked?: string;
    membersOnly?: IJunctoConference;
    metadata?: IConferenceMetadata;
    obfuscatedRoom?: string;
    obfuscatedRoomSource?: string;
    p2p?: Object;
    password?: string;
    passwordRequired?: IJunctoConference;
...
export interface IJunctoConferenceRoom {
    locked: boolean;
    myroomjid: string;
    roomjid: string;
    xmpp: {
...
function _conferenceFailed(state: IConferenceState, { conference, error }: {
    conference: IJunctoConference; error: IConferenceFailedError; }) {
    ...
    switch (error.name) {
    case JunctoConferenceErrors.AUTHENTICATION_REQUIRED:
     ...
     */
    case JunctoConferenceErrors.CONFERENCE_ACCESS_DENIED:
    case JunctoConferenceErrors.DISPLAY_NAME_REQUIRED: {
        lobbyError = true;

        break;
    }

    case JunctoConferenceErrors.MEMBERS_ONLY_ERROR: {
    ...
    case JunctoConferenceErrors.PASSWORD_REQUIRED:
    ...
    return assign(state, {
        ...
        /**
         * The JunctoConference instance which requires a password to join.
         *
         * @type {JunctoConference}
         */
        passwordRequired
    });
}
 ...
 * reduction of the specified action.
 */
function _conferenceJoined(state: IConferenceState, { conference }: { conference: IJunctoConference; }) {
    // FIXME The indicator which determines whether a JunctoConference is locked
    // i.e. password-protected is private to lib-juncto. However, the
    // library does not fire LOCK_STATE_CHANGED upon joining a JunctoConference
    // with a password.
    // FIXME Technically JunctoConference.room is a private field.
    ...
    return assign(state, {
        ...
        /**
         * The JunctoConference instance represented by the Redux state of the
...
{ conference: IJunctoConference; type: string; }) {
    const nextState = { ...state };

    // The redux action CONFERENCE_LEFT is the last time that we should be
    // hearing from a JunctoConference instance.
    //
    // The redux action CONFERENCE_WILL_LEAVE represents the order of the user
    // to leave a JunctoConference instance. From the user's perspective, there's
    ...
    if (type === CONFERENCE_WILL_LEAVE) {
        ...
        if (conference === state.joining || conference === state.conference) {
            /**
             * The JunctoConference instance which is currently in the process of
             * being left.
             *
             * @type {JunctoConference}
             */
 ...
 * reduction of the specified action.
 */
function _conferenceWillJoin(state: IConferenceState, { conference }: { conference: IJunctoConference; }) {
    return assign(state, {
        error: undefined,
        joining: conference
    });
...
function _setPassword(state: IConferenceState, { conference, method, password }: {
    conference: IJunctoConference; method: Object; password: string; }) {
    switch (method) {
    case conference.join:
        return assign(state, {
            // 1. The JunctoConference which transitions away from
            ...
            // JunctoConference whatsoever.
            //
            // 2. The redux action setPassword will attempt to join the
            // JunctoConference so joining is an appropriate transitional
            // redux state.
            //
