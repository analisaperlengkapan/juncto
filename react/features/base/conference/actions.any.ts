...
import { JUNCTO_CONNECTION_CONFERENCE_KEY } from '../connection/constants';
import { hasAvailableDevices } from '../devices/functions.any';
import JunctoMeetJS, { JunctoConferenceEvents, JunctoE2ePingEvents } from '../lib-juncto';
import {
...
import { IJunctoParticipant } from '../participants/types';
...
import { IConferenceMetadata, IJunctoConference } from './reducer';

/**
 * Adds conference (event) listeners.
 *
 * @param {JunctoConference} conference - The JunctoConference instance.
 * @param {Dispatch} dispatch - The Redux dispatch function.
 * @param {Object} state - The Redux state.
 * @private
 * @returns {void}
 */
function _addConferenceListeners(conference: IJunctoConference, dispatch: IStore['dispatch'], state: IReduxState) {
    // A simple logger for conference errors received through
    // the listener. These errors are not handled now, but logged.
    conference.on(JunctoConferenceEvents.CONFERENCE_ERROR,
    ...
    conference.on(JunctoConferenceEvents.SUBJECT_CHANGED,
        (subject: string) => dispatch(conferenceSubjectChanged(subject)));

    conference.on(JunctoConferenceEvents.CONFERENCE_CREATED_TIMESTAMP,
        (timestamp: number) => dispatch(conferenceTimestampChanged(timestamp)));

    conference.on(
        JunctoConferenceEvents.KICKED,
    ...
    conference.on(
        JunctoConferenceEvents.PARTICIPANT_KICKED,
        (kicker: any, kicked: any) => dispatch(participantKicked(kicker, kicked)));

    conference.on(
        JunctoConferenceEvents.PARTICIPANT_SOURCE_UPDATED,
        (junctoParticipant: IJunctoParticipant) => dispatch(participantSourcesUpdated(junctoParticipant)));
    ...
    conference.on(
        JunctoConferenceEvents.TRACK_ADDED,
        (t: any) => t && !t.isLocal() && dispatch(trackAdded(t)));
    conference.on(
        JunctoConferenceEvents.TRACK_REMOVED,
    ...
    conference.on(JunctoConferenceEvents.TRACK_UNMUTE_REJECTED, (track: any) => dispatch(destroyLocalTracks(track)));
 ...
 *
 * @param {JunctoConference} conference - The JunctoConference that has failed.
 ...
 *     conference: JunctoConference,
 *     error: Error
 * }}
 * @public
 */
export function conferenceFailed(conference: IJunctoConference, error: string, ...params: any) {
    return {
        type: CONFERENCE_FAILED,
        conference,
 ...
 *
 * @param {JunctoConference} conference - The JunctoConference instance which was
 ...
 * }}
 */
export function conferenceJoined(conference: IJunctoConference) {
    return {
        type: CONFERENCE_JOINED,
        conference
    };
 ...
 *
 * @param {JunctoConference} conference - The JunctoConference instance for which join by the local participant
 * is in progress.
 * @returns {{
 ...
 * }}
 */
export function conferenceJoinInProgress(conference: IJunctoConference) {
    return {
        type: CONFERENCE_JOIN_IN_PROGRESS,
        conference
    };
 ...
 * }}
 */
export function conferenceLeft(conference?: IJunctoConference) {
    return {
        type: CONFERENCE_LEFT,
        conference
    };
...
*
* @param {JunctoConference} conference - The JunctoConference instance, where the uuid has been set.
* @returns {{
*   type: CONFERENCE_UNIQUE_ID_SET,
*   conference: JunctoConference,
* }}
*/
export function conferenceUniqueIdSet(conference: IJunctoConference) {
    return {
        type: CONFERENCE_UNIQUE_ID_SET,
        conference
    };
 ...
 *
 * @param {JunctoConference} conference - The {@code JunctoConference} instance
 * the local participant will (try to) join.
 * @returns {Function}
 */
export function _conferenceWillJoin(conference: IJunctoConference) {
 ...
 * }}
 */
export function conferenceWillJoin(conference?: IJunctoConference) {
    return {
        type: CONFERENCE_WILL_JOIN,
        conference
    };
 ...
 * lib-juncto and not the application.
 *
 * @param {JunctoConference} conference - The JunctoConference instance which will
 ...
 * }}
 */
export function conferenceWillLeave(conference?: IJunctoConference, isRedirect?: boolean) {
    return {
...
export function createConference(overrideRoom?: string | String) {
    return (dispatch: IStore['dispatch'], getState: IStore['getState']) => {
        ...
        const conference = connection.initJunctoConference(_room, getConferenceOptions(state));

        // @ts-ignore
        connection[JUNCTO_CONNECTION_CONFERENCE_KEY] = conference;

        conference[JUNCTO_CONFERENCE_URL_KEY] = locationURL;
 ...
 * {@link JunctoConferenceErrors.AUTHENTICATION_REQUIRED}. It means that Juncofo
 ...
 *
 * @param {JunctoConference} conference - The {@link JunctoConference} instance
 * for which the event is being signaled.
 * @param {JunctoParticipant} participant - The {@link JunctoParticipant}
 * instance which initiated the kick event.
 * @returns {{
 *     type: KICKED_OUT,
 *     conference: JunctoConference,
 *     participant: JunctoParticipant
 * }}
 */
export function kickedOut(conference: IJunctoConference, participant: Object) {
    return {
        type: KICKED_OUT,
        conference,
        participant
...
/**
 * Signals that the lock state of a specific JunctoConference changed.
 *
 * @param {JunctoConference} conference - The JunctoConference which had its lock
 * state changed.
 ...
 * }}
 */
export function lockStateChanged(conference: IJunctoConference, locked: boolean) {
    return {
        type: LOCK_STATE_CHANGED,
        conference,
        locked
...
/**
 * Sets the password to join or lock a specific JunctoConference.
 *
 * @param {JunctoConference} conference - The JunctoConference which requires a
 * password to join or is to be locked with the specified password.
 * @param {Function} method - The JunctoConference method of password protection
...
export function setPassword(
        conference: IJunctoConference | undefined,
