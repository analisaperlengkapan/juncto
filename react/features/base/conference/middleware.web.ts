...
import { JunctoConferenceErrors, JunctoConnectionErrors, browser } from '../lib-juncto';
...
MiddlewareRegistry.register(store => next => action => {
    ...
    case CONFERENCE_FAILED: {
        ...
        if (enableForcedReload
            && (errorName === JunctoConferenceErrors.CONFERENCE_RESTARTED
                || errorName === JunctoConnectionErrors.SHARD_CHANGED_ERROR)) {
            dispatch(setSkipPrejoinOnReload(true));
        }

        if (errorName === JunctoConferenceErrors.CONFERENCE_DESTROYED) {
        ...
        if (prejoinVisible) {
            promise.then(() => {
                ...
                for (const track of localTracks) {
                    ...
                    if ((track.muted && !(browser.isWebKitBased() && track.junctoTrack
                            && track.junctoTrack.getType() === MEDIA_TYPE.AUDIO)) || iAmVisitor(state)) {
                        trackReplacePromises.push(dispatch(replaceLocalTrack(track.junctoTrack, null))
                            .catch((error: any) => {
                                logger.error(`Failed to replace local track (${track.junctoTrack}) with null: ${error}`);
                            }));
                    }
                }
                ...
                Promise.allSettled(trackReplacePromises).then(() => {
                    ...
                    const junctoTracks = localTracks.map((t: any) => t.junctoTrack);


                    return APP.conference.startConference(junctoTracks);
                })
                .catch(logger.error);
            });
        } else {
