...
import JunctoMeetJS from '../lib-juncto';
...
const webrtcKindToJunctoKindTranslator = {
...
export function getAudioOutputDeviceId() {
    return JunctoMeetJS.mediaDevices.getAudioOutputDevice();
}
...
export function getDefaultDeviceId(state: IReduxState, kind: string) {
    const kindToSearch = webrtcKindToJunctoKindTranslator[kind as keyof typeof webrtcKindToJunctoKindTranslator] || kind;
    const availableDevices = state['features/base/devices'].availableDevices;
...
export function setAudioOutputDeviceId(
    ...
    if (!JunctoMeetJS.mediaDevices.isDeviceChangeAvailable('output')) {
        logger.warn('Adjusting audio output is not supported');

        return Promise.resolve();
    }

    return JunctoMeetJS.mediaDevices.setAudioOutputDevice(newId)
        .then(() => {
