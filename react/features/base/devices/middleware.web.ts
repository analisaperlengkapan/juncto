...
import JunctoMeetJS, { JunctoMediaDevicesEvents, JunctoTrackErrors } from '../lib-juncto';
...
const JUNCTO_TRACK_ERROR_TO_MESSAGE_KEY_MAP = {
    microphone: {
        [JunctoTrackErrors.CONSTRAINT_FAILED]: 'dialog.micConstraintFailedError',
        [JunctoTrackErrors.GENERAL]: 'dialog.micUnknownError',
        [JunctoTrackErrors.NOT_FOUND]: 'dialog.micNotFoundError',
        [JunctoTrackErrors.PERMISSION_DENIED]: 'dialog.micPermissionDeniedError',
        [JunctoTrackErrors.TIMEOUT]: 'dialog.micTimeoutError'
    },
    camera: {
        [JunctoTrackErrors.CONSTRAINT_FAILED]: 'dialog.cameraConstraintFailedError',
        [JunctoTrackErrors.GENERAL]: 'dialog.cameraUnknownError',
        [JunctoTrackErrors.NOT_FOUND]: 'dialog.cameraNotFoundError',
        [JunctoTrackErrors.PERMISSION_DENIED]: 'dialog.cameraPermissionDeniedError',
        [JunctoTrackErrors.UNSUPPORTED_RESOLUTION]: 'dialog.cameraUnsupportedResolutionError',
        [JunctoTrackErrors.TIMEOUT]: 'dialog.cameraTimeoutError'
    }
};

/**
 * A listener for device permissions changed reported from lib-juncto.
 */
...
MiddlewareRegistry.register(store => next => action => {
    ...
    case APP_WILL_MOUNT: {
        ...
        mediaDevices.addEventListener(JunctoMediaDevicesEvents.PERMISSIONS_CHANGED, permissionsListener);
        Promise.all([
    ...
    case APP_WILL_UNMOUNT:
        if (typeof permissionsListener === 'function') {
            JunctoMeetJS.mediaDevices.removeEventListener(
                JunctoMediaDevicesEvents.PERMISSIONS_CHANGED, permissionsListener);
    ...
    case NOTIFY_CAMERA_ERROR: {
        ...
        const cameraJunctoTrackErrorMsg
            = JUNCTO_TRACK_ERROR_TO_MESSAGE_KEY_MAP.camera[name];
        const cameraErrorMsg = cameraJunctoTrackErrorMsg
            || JUNCTO_TRACK_ERROR_TO_MESSAGE_KEY_MAP
                .camera[JunctoTrackErrors.GENERAL];
        const additionalCameraErrorMsg = cameraJunctoTrackErrorMsg ? null : message;
        const titleKey = name === JunctoTrackErrors.PERMISSION_DENIED
    ...
    case NOTIFY_MIC_ERROR: {
        ...
        const micJunctoTrackErrorMsg
            = JUNCTO_TRACK_ERROR_TO_MESSAGE_KEY_MAP.microphone[name];
        const micErrorMsg = micJunctoTrackErrorMsg
            || JUNCTO_TRACK_ERROR_TO_MESSAGE_KEY_MAP
                .microphone[JunctoTrackErrors.GENERAL];
        const additionalMicErrorMsg = micJunctoTrackErrorMsg ? null : message;
        const titleKey = name === JunctoTrackErrors.PERMISSION_DENIED
    ...
    case SET_VIDEO_INPUT_DEVICE: {
        ...
        if (isMobileBrowser() && localTrack && !localTrack.muted) {
            localTrack.junctoTrack.stopStream();
        }
        if (isPrejoinPageVisible(store.getState())) {
