 ...
 * JunctoMeetJS#init(). The app will download config.js from the Juncto
 ...
 * environment. The mandatory configuration to be passed to JunctoMeetJS#init().
 * The app will download config.js from the Juncto deployment and take its
 ...
 * configuration to be passed to JunctoMeetJS#init(). The app will download
 * config.js from the Juncto deployment and take its values into account but
...
function _setConfig(state: IConfig, { config }: { config: IConfig; }) {
    ...
    const newState = merge(
        ...
        // downloaded from the Juncto deployment because the former contains
 ...
 * interface_config {@code Object} which is in the latest format supported by juncto.
 *
 * @param {Object} oldValue - The config {@code Object} which may or may not be
 * in the latest form supported by juncto and from which a new config
...
/**
 * Constructs a new config {@code Object}, if necessary, out of a specific
 * config {@code Object} which is in the latest format supported by juncto.
 * Such a translation from an old config format to a new/the latest config
 * format is necessary because the mobile app bundles juncto and
 * lib-juncto at build time and does not download them at runtime from the
