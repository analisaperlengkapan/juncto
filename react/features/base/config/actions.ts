...
/**
 * Signals that the configuration (commonly known in Juncto as config.js)
...
/**
 * Signals that a configuration (commonly known in Juncto as config.js)
 ...
 * configuration is defined and consumed by the library lib-juncto but some
 * of its properties are consumed by the application juncto as well.
 *
...
/**
 * Stores a specific Juncto config.js object into {@code localStorage}.
 *
 * @param {string} baseURL - The base URL from which the config.js was
 * downloaded.
 * @param {Object} config - The Juncto config.js to store.
 * @returns {Function}
 */
    ...
    return (dispatch: IStore['dispatch']) => {
        ...
        try {
            if (typeof window.config === 'undefined' || window.config !== config) {
                junctoLocalStorage.setItem(`${_CONFIG_STORE_PREFIX}/${baseURL}`, JSON.stringify(config));
                b = true;
            }
        } catch (e) {
