...
/**
 * Restores a Juncto config.js from {@code localStorage} if it was
 ...
 * @returns {?Object} The Juncto config.js which was previously downloaded
...
export function restoreConfig(baseURL: string) {
    ...
    const config = junctoLocalStorage.getItem(key);
    ...
    if (config) {
        try {
            ...
            junctoLocalStorage.removeItem(key);
        }
    }
...
export function setConfigFromURLParams(
    ...
    //     "interfaceConfig.APP_NAME": "Juncto"
    // }
    // We want to have:
    // json = {
    //     config: {
