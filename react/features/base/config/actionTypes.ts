/**
 * The redux action which signals that a configuration (commonly known in Juncto
 ...
 * lib-juncto but some of its properties are consumed by the application
 * juncto as well.
 *
 * {
 *     type: SET_CONFIG,
 ...
 * base/config. The configuration is defined and consumed by the library
 * lib-juncto but some of its properties are consumed by the application
 * juncto as well. A merge operation is performed between the existing config
