...
import JunctoMeetJS, { JunctoConnectionEvents } from '../lib-juncto';
...
import { JUNCTO_CONNECTION_URL_KEY } from './constants';
import logger from './logger';
import { ConnectionFailedError, IIceServers } from './types';

/**
 * The options that will be passed to the JunctoConnection instance.
 */
 ...
 *
 * @param {JunctoConnection} connection - The {@code JunctoConnection} which
 * disconnected.
 * @private
 * @returns {{
 *     type: CONNECTION_DISCONNECTED,
 *     connection: JunctoConnection
 * }}
 */
 ...
 *
 * @param {JunctoConnection} connection - The {@code JunctoConnection} which was
 * established.
 * @param {number} timeEstablished - The time at which the
 * {@code JunctoConnection} which was established.
 * @public
 * @returns {{
...
/**
 * Constructs options to be passed to the constructor of {@code JunctoConnection}
 * based on the redux state.
 *
 * @param {Object} state - The redux state.
 * @returns {Object} The options to be passed to the constructor of
 * {@code JunctoConnection}.
 */
export function constructOptions(state: IReduxState) {
    ...
    // Allow iceServersOverride only when juncto is in an iframe.
...
/**
 * To change prefer visitor in the store. Used later to decide what to request from juncofo on connection.
 *
...
export function _connectInternal(id?: string, password?: string) {
    return (dispatch: IStore['dispatch'], getState: IStore['getState']) => {
        ...
        const connection = new JunctoMeetJS.JunctoConnection(options.appId, jwt, options);

        connection[JUNCTO_CONNECTION_URL_KEY] = locationURL;

        dispatch(_connectionWillConnect(connection));

        return new Promise((resolve, reject) => {
            connection.addEventListener(
                JunctoConnectionEvents.CONNECTION_DISCONNECTED,
                _onConnectionDisconnected);
            connection.addEventListener(
                JunctoConnectionEvents.CONNECTION_ESTABLISHED,
                _onConnectionEstablished);
            connection.addEventListener(
                JunctoConnectionEvents.CONNECTION_FAILED,
                _onConnectionFailed);
            connection.addEventListener(
                JunctoConnectionEvents.CONNECTION_REDIRECTED,
                _onConnectionRedirected);
            connection.addEventListener(
                JunctoConnectionEvents.PROPERTIES_UPDATED,
                _onPropertiesUpdate);
            ...
            function unsubscribe() {
                connection.removeEventListener(
                    JunctoConnectionEvents.CONNECTION_DISCONNECTED, _onConnectionDisconnected);
                connection.removeEventListener(JunctoConnectionEvents.CONNECTION_FAILED, _onConnectionFailed);
                connection.removeEventListener(JunctoConnectionEvents.PROPERTIES_UPDATED, _onPropertiesUpdate);
            }
             ...
             *
             * @param {JunctoConnectionErrors} err - Connection error.
             * @param {string} [message] - Error message supplied by lib-juncto.
            ...
            function _onConnectionEstablished() {
                connection.removeEventListener(JunctoConnectionEvents.CONNECTION_ESTABLISHED, _onConnectionEstablished);
            ...
            function _onConnectionRedirected(vnode: string, focusJid: string, username: string) {
                connection.removeEventListener(JunctoConnectionEvents.CONNECTION_REDIRECTED, _onConnectionRedirected);
 ...
 *
 * @param {JunctoConnection} connection - The {@code JunctoConnection} which will
 * connect.
 * @private
 * @returns {{
 ...
 * @param {boolean} shouldLeave - Indicates whether to call JunctoConference.leave().
 * @returns {Function}
 */
export function disconnect(isRedirect?: boolean, shouldLeave = true) {
    return (dispatch: IStore['dispatch'], getState: IStore['getState']): Promise<void> => {
        ...
        if (conference_) {
            // In a fashion similar to JunctoConference's CONFERENCE_LEFT event
            ...
            if (!shouldLeave) {
                // we are skipping JunctoConference.leave(), but will still dispatch the normal leave flow events
                ...
                promise
                    ...
                    .catch((error: Error) => {
                        logger.warn(
                            'JunctoConference.leave() rejected with:',
                            error);

                        // The library lib-juncto failed to make the
                        // JunctoConference leave. Which may be because
                        // JunctoConference thinks it has already left.
                        // Regardless of the failure reason, continue in
                        // juncto as if the leave has succeeded.
                        dispatch(conferenceLeft(conference_));
                    });
            }
        } else {
