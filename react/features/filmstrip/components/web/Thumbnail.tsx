...
class Thumbnail extends Component<IProps, IState> {
    ...
    override componentDidMount() {
        ...
        if (_videoTrack && !_videoTrack.local) {
            _videoTrack.junctoTrack.on(JunctoTrackEvents.TRACK_STREAMING_STATUS_CHANGED,
                this.handleTrackStreamingStatusChanged);
            dispatch(trackStreamingStatusChanged(_videoTrack.junctoTrack,
                _videoTrack.junctoTrack.getTrackStreamingStatus()));
        }
    }
    ...
    override componentWillUnmount() {
        ...
        if (_videoTrack && !_videoTrack.local) {
            _videoTrack.junctoTrack.off(JunctoTrackEvents.TRACK_STREAMING_STATUS_CHANGED,
                this.handleTrackStreamingStatusChanged);
            dispatch(trackStreamingStatusChanged(_videoTrack.junctoTrack,
                _videoTrack.junctoTrack.getTrackStreamingStatus()));
        }
    ...
    override componentDidUpdate(prevProps: IProps, prevState: IState) {
        ...
        if (prevProps._videoTrack?.junctoTrack?.getSourceName() !== _videoTrack?.junctoTrack?.getSourceName()) {
            if (prevProps._videoTrack && !prevProps._videoTrack.local) {
                prevProps._videoTrack.junctoTrack.off(JunctoTrackEvents.TRACK_STREAMING_STATUS_CHANGED,
                    this.handleTrackStreamingStatusChanged);
                dispatch(trackStreamingStatusChanged(prevProps._videoTrack.junctoTrack,
                    prevProps._videoTrack.junctoTrack.getTrackStreamingStatus()));
            }
            if (_videoTrack && !_videoTrack.local) {
                _videoTrack.junctoTrack.on(JunctoTrackEvents.TRACK_STREAMING_STATUS_CHANGED,
                    this.handleTrackStreamingStatusChanged);
     ...
     * @returns {void}
     */
    handleTrackStreamingStatusChanged(junctoTrack: any, streamingStatus: any) {
        this.props.dispatch(trackStreamingStatusChanged(junctoTrack, streamingStatus));
    }
    ...
    _getStyles(): any {
        ...
        const isTileType = _thumbnailType === THUMBNAIL_TYPE.TILE;
        const junctoVideoTrack = _videoTrack?.junctoTrack;
        const track = junctoVideoTrack?.track;
    ...
    _renderParticipant(local = false) {
        ...
        const junctoVideoTrack = _videoTrack?.junctoTrack;
        const videoTrackId = junctoVideoTrack?.getId();
