...
import { JunctoConferenceErrors } from '../base/lib-juncto';
...
function _conferenceFailed({ dispatch }: IStore, next: Function, action: AnyAction) {
    ...
    if (conference && error.name === JunctoConferenceErrors.PASSWORD_REQUIRED) {
        // XXX The feature room-lock affords recovery after CONFERENCE_FAILED
        // caused by JunctoConferenceErrors.PASSWORD_REQUIRED.
...
function _setPasswordFailed(store: IStore, next: Function, action: AnyAction) {
    if (typeof APP !== 'undefined') {
        ...
        if (error === JunctoConferenceErrors.PASSWORD_NOT_SUPPORTED) {
