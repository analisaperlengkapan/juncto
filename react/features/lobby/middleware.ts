...
StateListenerRegistry.register(
    ...
    (conference, { dispatch, getState }, previousConference) => {
        if (conference && !previousConference) {
            conference.on(JunctoConferenceEvents.MEMBERS_ONLY_CHANGED, (enabled: boolean) => {
            ...
            conference.on(JunctoConferenceEvents.LOBBY_USER_JOINED, (id: string, name: string) => {
            ...
            conference.on(JunctoConferenceEvents.LOBBY_USER_UPDATED, (id: string, participant: IKnockingParticipant) => {
                dispatch(
                    participantIsKnockingOrUpdated({
                        ...participant,
                        id
            ...
            conference.on(JunctoConferenceEvents.LOBBY_USER_LEFT, (id: string) => {
                batch(() => {
...
function _conferenceFailed({ dispatch, getState }: IStore, next: Function, action: AnyAction) {
    ...
    if (error.name === JunctoConferenceErrors.MEMBERS_ONLY_ERROR) {
        ...
        return result;
    } else if (error.name === JunctoConferenceErrors.DISPLAY_NAME_REQUIRED) {
    ...
    if (error.name === JunctoConferenceErrors.CONFERENCE_ACCESS_DENIED) {
        dispatch(
            showNotification({
