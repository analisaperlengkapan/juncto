...
StateListenerRegistry.register(
    ...
    (conference, { dispatch, getState }, previousConference) => {
        if (conference && !previousConference) {
            conference.on(JunctoConferenceEvents.FILE_SHARING_FILE_ADDED, (file: IFileMetadata) => {
            ...
            conference.on(JunctoConferenceEvents.FILE_SHARING_FILE_REMOVED, (fileId: string) => {
                const state = getState();
                const localParticipant = getLocalParticipant(state);
            ...
            conference.on(JunctoConferenceEvents.FILE_SHARING_FILES_RECEIVED, (files: object) => {
                const state = getState();
                const localParticipant = getLocalParticipant(state);
