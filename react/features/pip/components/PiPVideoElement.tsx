...
const PiPVideoElement: React.FC = () => {
    ...
    useEffect(() => {
        ...
        if (previousTrack?.junctoTrack) {
            try {
                previousTrack.junctoTrack.detach(videoElement);
            } catch (error) {
        ...
        } else if (videoTrack?.junctoTrack) {
            // Attach real video track.
            videoTrack.junctoTrack.attach(videoElement)
                .catch((error: Error) => {
        ...
        return () => {
            if (videoTrack?.junctoTrack && videoElement) {
                try {
                    videoTrack.junctoTrack.detach(videoElement);
                } catch (error) {
