...
DEFAULT_MVN_REPO="${THIS_DIR}/../../../juncto-maven-repository/releases"
...
echo "Using ${MVN_REPO} as the Maven repo"
 ...
 if [[ $MVN_HTTP == 0 ]]; then
    ...
    if [[ -d ${MVN_REPO}/org/juncto/react/juncto-sdk/${SDK_VERSION} ]]; then
        echo "There is already a release with that version in the Maven repo!"
        exit 1
    fi
fi

# Now build and publish the Juncto SDK and its dependencies
echo "Building and publishing the Juncto SDK"
pushd ${THIS_DIR}/../
./gradlew clean
...
if [[ $MVN_HTTP == 0 ]]; then
    pushd ${MVN_REPO_PATH}
    git add -A .
    git commit -m "Juncto SDK + dependencies: ${SDK_VERSION}"
    popd
fi
