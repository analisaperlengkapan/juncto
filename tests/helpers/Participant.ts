...
export class Participant {
    ...
    /**
     * Whether the current frame is the main frame. This could coincide with the Juncto frame (when it's loaded
    ...
    /**
     * Switches to the main frame context (outside the iFrame; where the Juncto iFrame API is available).
     *
    ...
    /**
     * Switches to the iFrame context (inside the iFrame; where the Juncto application runs).
     *
     * If this Participant was initialized with iFrameApi=false this is a no-op.
     */
    ...
    async isRemoteVideoReceived(endpointId: string): Promise<boolean> {
        return this.execute(e => JunctoMeetJS.app.testing.isRemoteVideoReceived(e), endpointId);
    }
