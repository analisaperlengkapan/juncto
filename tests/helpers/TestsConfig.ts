...
export const config = {
    ...
    debug: Boolean(process.env.JUNCTO_DEBUG?.trim()),
    ...
    jaas: {
        ...
        /** Whether the configuration for JunctoService specific tests is enabled. */
        enabled: Boolean(
        ...
        /** The JunctoService key ID, used to sign the tokens. */
        kid: (() => {
        ...
        /** The path to the JunctoService private key, used to sign JunctoService tokens. */
        privateKeyPath: (() => {
        ...
        /** The JunctoService tenant (vpaas-magic-cookie-<ID>) . */
        tenant: (() => {
            if (typeof process.env.JAAS_TENANT !== 'undefined') {
