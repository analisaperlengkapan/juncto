...
// we need it to be able to reuse juncto code in tests
...
export const config: WebdriverIO.MultiremoteConfig = {
    ...
    baseUrl: process.env.BASE_URL || 'https://alpha.juncto.net/torture/',
    ...
    async before(cid, _, files) {
        ...
        if (testProperties.useJaas && !testsConfig.jaas.enabled) {
            globalAny.ctx.skipSuiteTests = 'JunctoService is not configured';
        ...
        if (testProperties.useWebhookProxy && testsConfig.webhooksProxy.enabled && !globalAny.ctx.webhooksProxy) {
            ...
            if (!testProperties.useJaas) {
                throw new Error('The test tries to use WebhookProxy without JunctoService.');
            }
            if (!tenant) {
    ...
    beforeTest(test, context) {
        ...
        if (file) {
            AllureReporter.addLink(`https://github.com/juncto/juncto/blob/master/tests/specs/${file}`, 'Code');
        }
