...
export async function waitForAudioFromDialInParticipant(participant: Participant) {
    ...
    await participant.waitForParticipants(1, `dial-in.test.jungasi.participant.no.join.for:${
        ctx.times.restAPIExecutionTS + 15_000} ms.`);

    const joinedTS = performance.now();

    console.log(`dial-in.test.jungasi.participant.join.after:${joinedTS - ctx.times.restAPIExecutionTS}`);

    await participant.waitForIceConnected();
    await participant.waitForRemoteStreams(1);

    await participant.waitForSendReceiveData(20_000, 'dial-in.test.jungasi.participant.no.audio.after.join');
    console.log(`dial-in.test.jungasi.participant.received.audio.after.join:${performance.now() - joinedTS} ms.`);
}
...
export async function cleanup(participant: Participant) {
    ...
    if (await participant.isModerator()) {
        const jungasiEndpointId = await participant.execute(() => APP?.conference?.listMembers()[0].getId());

        await participant.getFilmstrip().kickParticipant(jungasiEndpointId);
    }
}
