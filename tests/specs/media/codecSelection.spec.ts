...
describe('Codec selection', () => {
    it('asymmetric codecs', async () => {
        ...
        expect(await p1.execute(() => JunctoMeetJS.app.testing.isLargeVideoReceived())).toBe(true);
    ...
    it('asymmetric codecs with AV1', async () => {
        ...
        expect(await p3.execute(() => JunctoMeetJS.app.testing.isLargeVideoReceived())).toBe(true);
...
async function waitForCodec(p: Participant, codec: string) {
    await p.driver.waitUntil(
        () => p.execute(c => JunctoMeetJS.app.testing.getLocalCameraEncoding() === c, codec),
        {
            timeout: 10000,
