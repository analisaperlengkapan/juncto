...
describe('Desktop sharing', () => {
    it('start', async () => {
        ...
        expect(await p2.execute(() => JunctoMeetJS.app.testing.isLargeVideoReceived())).toBe(true);
    });
    ...
    /**
     * Ensures screen share is still visible when the call switches from p2p to junctobridge connection.
     */
    it('p2p to junctobridge switch', async () => {
    ...
    /**
     * Ensure screen share is still visible when the call switches from junctobridge to p2p and back.
     */
    it('p2p to junctobridge switch and back', async () => {
        const { p1, p2, p3 } = ctx;
    ...
    /**
     * Ensure that screen share is still visible in junctobridge connection when share is toggled while the users are
     * in p2p mode, i.e., share is restarted when user is in p2p mode and then the call switches over to junctobridge mode.
     */
     ...
     *  The call switches to junctobridge and then back to p2p.
     */
    it('screen sharing toggle before others join', async () => {
        ...
        // p1 starts share again when call switches to junctobridge.
        await p1.getToolbar().clickDesktopSharingButton();
     ...
     * where only a screen share can be received. A bug fixed in junctobridge 0c5dd91b where the video was not received.
     */
    it('audio only and non dominant screen share', async () => {
        ...
        // the video should be playing
        await p1.driver.waitUntil(() => p1.execute(() => JunctoMeetJS.app.testing.isLargeVideoReceived()), {
            timeout: 5_000,
     ...
     * A problem fixed in juncto 3657c19e and d6ab0a72.
     */
    ...
    it('with lastN', async () => {
        ...
        expect(await p4.execute(() => JunctoMeetJS.app.testing.isLargeVideoReceived())).toBe(true);
